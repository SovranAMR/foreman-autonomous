...
export interface FormalStateMachineProbeRegressionReport {
...
/**
 * Compare formal state machine run records and detect probe alignment regressions.
 * A regression = probe aligned in prior run but misaligned in current run.
 */
export function detectFormalStateMachineProbeRegression(
  prior: FormalStateMachineRunRecord,
  current: FormalStateMachineRunRecord,
...
// ─── Guard controls (P01-B03-A09 foundation, used by A08 regression gate) ───

export interface ForgeFormalStateMachineGuardControls {
  atom: string;
  adversarial: {
...
export interface FormalStateMachineGuardCheckIssue {
  domain: "adversarial" | "performance" | "cost" | "safety";
  code: string;
  detail: string;
}

export interface FormalStateMachineGuardCheckResult {
  passed: boolean;
  issues: FormalStateMachineGuardCheckIssue[];
  metrics: {
...
export interface FormalStateMachineAdversarialGuardScenario {
  id: string;
  description: string;
  build: (record: FormalStateMachineRunRecord) => FormalStateMachineRunRecord;
  expectRejected: true;
}

export const FORGE_FORMAL_STATE_MACHINE_GUARD_CONTROLS_V1: ForgeFormalStateMachineGuardControls = {
  atom: "P01-B03-A09",
  adversarial: {
    rejectTamperedRecords: true,
    rejectFalseAlignment: true,
...
export function getForgeFormalStateMachineGuardControls(): ForgeFormalStateMachineGuardControls {
  return FORGE_FORMAL_STATE_MACHINE_GUARD_CONTROLS_V1;
}

function parseFormalStateMachineIsoDurationMs(startedAt: string, completedAt: string): number {
...
export function summarizeFormalStateMachineTelemetry(telemetry: FormalStateMachineProbeTelemetry[]): {
  suiteDurationMs: number;
  maxProbeDurationMs: number;
} {
...
export function detectFormalStateMachineEvidenceSummaryMismatch(
  record: FormalStateMachineRunRecord,
): string | null {
  let alignedCount = 0;
  for (const item of record.evidence) {
...
export function detectFormalStateMachineFalseAlignment(record: FormalStateMachineRunRecord): string[] {
...
export function validateFormalStateMachineSafety(
  record: FormalStateMachineRunRecord,
  controls: ForgeFormalStateMachineGuardControls = getForgeFormalStateMachineGuardControls(),
): FormalStateMachineGuardCheckIssue[] {
  const issues: FormalStateMachineGuardCheckIssue[] = [];
  for (const item of record.evidence) {
...
export function validateFormalStateMachinePerformance(
  record: FormalStateMachineRunRecord,
  controls: ForgeFormalStateMachineGuardControls = getForgeFormalStateMachineGuardControls(),
): FormalStateMachineGuardCheckIssue[] {
  const issues: FormalStateMachineGuardCheckIssue[] = [];
  const { suiteDurationMs, maxProbeDurationMs } = summarizeFormalStateMachineTelemetry(record.telemetry);
  const wallClockMs = parseFormalStateMachineIsoDurationMs(
...
export function validateFormalStateMachineCost(
  totalCostUsd: number,
  llmCalls: number,
  controls: ForgeFormalStateMachineGuardControls = getForgeFormalStateMachineGuardControls(),
): FormalStateMachineGuardCheckIssue[] {
...
export function buildFormalStateMachineAdversarialGuardScenarios(): FormalStateMachineAdversarialGuardScenario[] {
  return [
    {
...
export function runFormalStateMachineAdversarialGuardChecks(
  fsmRecord: FormalStateMachineRunRecord,
  contract: FormalStateMachineContract = getActiveFormalStateMachineContract(),
  ...
  for (const scenario of scenarios) {
    const tampered = scenario.build(fsmRecord);
    const validation = validateFormalStateMachineRunRecord(tampered, contract);
    const falseAlignment = detectFormalStateMachineFalseAlignment(tampered);
    const summaryMismatch = detectFormalStateMachineEvidenceSummaryMismatch(tampered);
...
export function validateForgeFormalStateMachineGuard(
  record: FormalStateMachineRunRecord,
  options: {
    totalCostUsd?: number;
    llmCalls?: number;
    contract?: FormalStateMachineContract;
    controls?: ForgeFormalStateMachineGuardControls;
  } = {},
): FormalStateMachineGuardCheckResult {
  const controls = options.controls ?? getForgeFormalStateMachineGuardControls();
  const contract = options.contract ?? getActiveFormalStateMachineContract();
  const totalCostUsd = options.totalCostUsd ?? 0;
  const llmCalls = options.llmCalls ?? 0;
  const issues: FormalStateMachineGuardCheckIssue[] = [];

  issues.push(...validateFormalStateMachinePerformance(record, controls));
  issues.push(...validateFormalStateMachineCost(totalCostUsd, llmCalls, controls));
  issues.push(...validateFormalStateMachineSafety(record, controls));

  const falseAlignment = detectFormalStateMachineFalseAlignment(record);
  ...
  const summaryMismatch = detectFormalStateMachineEvidenceSummaryMismatch(record);
  if (summaryMismatch) {
    issues.push({
      domain: "adversarial",
  ...
  const adversarial = runFormalStateMachineAdversarialGuardChecks(record, contract);
  ...
  const telemetrySummary = summarizeFormalStateMachineTelemetry(record.telemetry);
  const wallClockMs = parseFormalStateMachineIsoDurationMs(
    record.provenance.startedAt,
    record.provenance.completedAt,
  );
...
// ─── Block gate and handoff (P01-B03-A10) ────────────────────────────────────

export interface FormalStateMachineBlockGateEvidence {
  blockId: string;
  atom: string;
  sealedAt: string;
...
export interface FormalStateMachineBlockHandoffContract {
  version: string;
  atom: string;
  sourceBlock: {
    blockId: string;
  ...
  sealedArtifacts: {
    ...
    harnessVersion: string;
    probeCount: number;
    fsmCategories: readonly FormalStateMachineCategory[];
    sourceBehaviorMapAtom: string;
  };
  ...
  entryCriteria: {
    ...
    formalStateMachineRecordRequired: true;
  };
}

export const FORGE_P01_B03_BLOCK_GATE_V1: ForgeBlockGateDefinition = {
  version: "1.0.0",
  atom: "P01-B03-A10",
  blockId: "P01-B03",
  ...
  checks: [
    { id: "fixture_contract_alignment", atomId: "P01-B03-A01", description: "Formal state machine fixture aligns with typed contract" },
    { id: "typed_contract_coverage", atomId: "P01-B03-A02", description: "Contract declares measurable probes for all FSM categories" },
    { id: "probe_matrix_aligned", atomId: "P01-B03-A03", description: "FSM probe matrix executes with zero mismatches" },
    { id: "boundary_disposition_coverage", atomId: "P01-B03-A04", description: "Contract covers observed, gap, failure, recovery and NO-GO dispositions" },
    { id: "failure_recovery_nogo", atomId: "P01-B03-A05", description: "Failure, recovery and NO-GO probes are declared and exercised" },
    { id: "evidence_telemetry_provenance", atomId: "P01-B03-A06", description: "Run record carries evidence, telemetry and provenance" },
    { id: "property_and_fuzz", atomId: "P01-B03-A07", description: "Structural property and fuzz validation reject tampered inputs" },
    { id: "regression_gate", atomId: "P01-B03-A08", description: "Regression gate passes on canonical FSM matrix" },
    { id: "guard_controls", atomId: "P01-B03-A09", description: "Adversarial, performance, cost and safety guard controls pass" },
    { id: "block_gate_sealed", atomId: "P01-B03-A10", description: "Block gate evidence sealed with valid B04 handoff contract" },
  ] satisfies readonly ForgeBlockGateCheck[],
};

export const FORGE_P01_B03_TO_B04_HANDOFF_V1: FormalStateMachineBlockHandoffContract = {
  version: "1.0.0",
  atom: "P01-B03-A10",
  sourceBlock: {
    blockId: "P01-B03",
    title: "Formal state machine",
    completedAtoms: FORGE_P01_B03_BLOCK_GATE_V1.requiredAtomIds,
  },
  targetBlock: {
    blockId: "P01-B04",
    title: "Typed phase/event schema",
    entryAtom: "P01-B04-A01",
  },
  sealedArtifacts: {
    fixtureVersion: "1.0.0",
    contractVersion: FORGE_FORMAL_STATE_MACHINE_CONTRACT_V1.version,
    harnessVersion: FORGE_FORMAL_STATE_MACHINE_HARNESS_VERSION,
    probeCount: summarizeFormalStateMachineContractCoverage(FORGE_FORMAL_STATE_MACHINE_CONTRACT_V1).totalProbes,
    fsmCategories: FORMAL_STATE_MACHINE_CATEGORIES,
    sourceBehaviorMapAtom: "P01-B02-A10",
  },
  prerequisites: [
    "Typed formal state machine contract v1 with measurable category invariants",
    "Versioned FSM fixture aligned to contract probe matrix",
    "Evidence, telemetry and provenance run records",
    "Regression and guard gates integrated with orchestrator verification",
    "Sealed P01-B02 behavior map artifacts referenced by sourceBehaviorMap",
  ],
  entryCriteria: {
    description:
      "B04-A01 formalizes typed phase/event schema using the sealed formal state machine artifacts",
    requiresBlockGatePass: true,
    formalStateMachineRecordRequired: true,
  },
};

export function getForgeP01B03BlockGate(): ForgeBlockGateDefinition {
  return FORGE_P01_B03_BLOCK_GATE_V1;
}

export function getForgeP01B03ToB04Handoff(): FormalStateMachineBlockHandoffContract {
  return FORGE_P01_B03_TO_B04_HANDOFF_V1;
}

export function validateFormalStateMachineBlockHandoffContract(
  handoff: FormalStateMachineBlockHandoffContract,
  evidence: Pick<FormalStateMachineBlockGateEvidence, "probeCount" | "regressionPassed" | "guardPassed">,
  contract: FormalStateMachineContract = getActiveFormalStateMachineContract(),
  ...
  if (handoff.sealedArtifacts.fsmCategories.length !== FORMAL_STATE_MACHINE_CATEGORIES.length) {
    issues.push("handoff fsmCategories incomplete");
  }
  if (handoff.targetBlock.entryAtom !== "P01-B04-A01") {
...
export function buildFormalStateMachineBlockGateEvidence(
  atomSeals: ForgeBlockAtomSeal[],
  regressionPassed: boolean,
  guardPassed: boolean,
  probeCount: number,
  gitCommit?: string,
  blockId = FORGE_P01_B03_BLOCK_GATE_V1.blockId,
): FormalStateMachineBlockGateEvidence {
  const handoff = getForgeP01B03ToB04Handoff();
  const handoffValid = validateFormalStateMachineBlockHandoffContract(handoff, {
    probeCount,
    regressionPassed,
    guardPassed,
  }).valid;
