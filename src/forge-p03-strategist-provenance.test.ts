...
import {
  ...
  buildStrategistProvenanceAdversarialGuardScenarios,
  detectStrategistProvenanceFalseAlignment,
  detectStrategistProvenanceEvidenceSummaryMismatch,
  runStrategistProvenanceAdversarialGuardChecks,
  validateForgeStrategistProvenanceGuard,
  validateStrategistProvenancePerformance,
  validateStrategistProvenanceCost,
  validateStrategistProvenanceSafety,
  getForgeStrategistProvenanceGuardControls,
...
describe("Forge Strategist Provenance Guard — P03-B09-A09 adversarial", () => {
  ...
  it("detects false alignment and summary/evidence mismatch", () => {
    const falsePassEvidence = buildStrategistProvenanceProbeEvidence(
    ...
    const fixture = loadStrategistProvenanceBaseline();
    const contract = getActiveStrategistProvenanceContract();
    const falsePassRecord = buildStrategistProvenanceRunRecord(
      buildStrategistProvenanceProvenance(
        "adv-false-pass",
        fixture,
        contract,
      ...
      [falsePassEvidence],
      [buildStrategistProvenanceProbeTelemetry("sprov.prompt_plan_provenance", "provenance_versioning", 0, 1)],
    );
    assert.ok(detectStrategistProvenanceFalseAlignment(falsePassRecord).length > 0);

    const summaryEvidence = buildStrategistProvenanceProbeEvidence(
      "sprov.prompt_plan_provenance",
      "provenance_versioning",
      "PASS",
      "FAIL",
    ...
    const summaryRecord = buildStrategistProvenanceRunRecord(
      buildStrategistProvenanceProvenance(
        "adv-summary",
        fixture,
        contract,
    ...
    assert.ok(detectStrategistProvenanceEvidenceSummaryMismatch(mismatchedSummary));
  });

  it("buildStrategistProvenanceAdversarialGuardScenarios cover false PASS attack vectors", () => {
...
describe("Forge Strategist Provenance Guard — P03-B09-A09 performance, cost, safety", () => {
  it("passes performance and zero-cost guard on canonical provenance run", () => {
    const record = runStrategistProvenanceProbesWithRecord();
    const contract = getActiveStrategistProvenanceContract();
    const guard = validateForgeStrategistProvenanceGuard(record, {
      totalCostUsd: 0,
      llmCalls: 0,
      contract,
    });
    ...
    assert.ok(
      guard.metrics.maxProbeDurationMs <
        getForgeStrategistProvenanceGuardControls().performance.maxProbeDurationMs,
    );
  ...
  it("flags cost and performance budget violations", () => {
    ...
    const telemetry = probeIds.map((id, index) => {
      const probe = contract.probes.find(p => p.id === id)!;
      return buildStrategistProvenanceProbeTelemetry(id, probe.category, index, 10_000);
    });
    ...
    const perfIssues = validateStrategistProvenancePerformance(record);
    assert.ok(perfIssues.some(i => i.domain === "performance"));

    const costIssues = validateStrategistProvenanceCost(0.05, 2);
  ...
  it("flags forbidden secret patterns in evidence detail", () => {
    const fixture = loadStrategistProvenanceBaseline();
    const contract = getActiveStrategistProvenanceContract();
    const evidence = buildStrategistProvenanceProbeEvidence(
      "sprov.prompt_plan_provenance",
      "provenance_versioning",
      "PASS",
      "PASS",
    ...
    const safetyIssues = validateStrategistProvenanceSafety(record);
