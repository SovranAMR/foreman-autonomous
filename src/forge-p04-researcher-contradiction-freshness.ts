...
// ─── Block gate and handoff (P04-B06-A10) ─────────────────────────────────────

export interface ResearcherContradictionFreshnessBlockGateEvidence {
  blockId: string;
  atom: string;
  sealedAt: string;
...
export interface ResearcherContradictionFreshnessBlockHandoffContract {
  version: string;
  atom: string;
  sourceBlock: {
    blockId: string;
  ...
  sealedArtifacts: {
    ...
    harnessVersion: string;
    probeCount: number;
    contradictionFreshnessCategories: readonly ResearcherContradictionFreshnessCategory[];
    sourceBlockGateAtom: string;
  };
  ...
  entryCriteria: {
    ...
    contradictionFreshnessRecordRequired: true;
  };
}

export const FORGE_P04_B06_BLOCK_GATE_V1: ForgeBlockGateDefinition = {
  version: "1.0.0",
  atom: "P04-B06-A10",
  blockId: "P04-B06",
  ...
  checks: [
    {
      ...
      atomId: "P04-B06-A01",
      description:
        "Contradiction freshness baseline aligns with typed contract and P04-B05 block gate handoff",
    },
    {
      id: "typed_contract_coverage",
      atomId: "P04-B06-A02",
      description: "Contract declares measurable probes for all contradiction freshness categories",
    },
    {
      id: "probe_matrix_aligned",
      atomId: "P04-B06-A03",
      description: "Contradiction freshness probe matrix executes with zero unexpected mismatches",
    },
    {
    ...
    },
    {
      id: "regression_gate",
      atomId: "P04-B06-A08",
      description: "Regression gate passes on canonical contradiction freshness matrix",
    },
    {
      id: "guard_controls",
      atomId: "P04-B06-A09",
...
export const FORGE_P04_B06_TO_B07_HANDOFF_V1: ResearcherContradictionFreshnessBlockHandoffContract = {
  version: "1.0.0",
  atom: "P04-B06-A10",
  sourceBlock: {
    blockId: "P04-B06",
    title: "Contradiction ve freshness çözümü",
    completedAtoms: FORGE_P04_B06_BLOCK_GATE_V1.requiredAtomIds,
  },
  targetBlock: {
    blockId: "P04-B07",
    title: "Risk ve trade-off araştırması",
    entryAtom: "P04-B07-A01",
  },
  sealedArtifacts: {
    fixtureVersion: "1.0.0",
    contractVersion: FORGE_RESEARCHER_CONTRADICTION_FRESHNESS_CONTRACT_V1.version,
    harnessVersion: FORGE_RESEARCHER_CONTRADICTION_FRESHNESS_VERSION,
    probeCount: summarizeResearcherContradictionFreshnessContractCoverage(
      FORGE_RESEARCHER_CONTRADICTION_FRESHNESS_CONTRACT_V1,
    ).totalProbes,
    contradictionFreshnessCategories: RESEARCHER_CONTRADICTION_FRESHNESS_CATEGORIES,
    sourceBlockGateAtom: "P04-B05-A10",
  },
  prerequisites: [
    "Contradiction freshness contract v1 with measurable contradiction, freshness and guard probes",
    "Versioned contradiction freshness baseline aligned to contract probe matrix and sealed P04-B05 block gate",
    "Evidence, telemetry and provenance run records",
    "Regression and guard gates integrated with orchestrator verification",
    "Sealed P04-B05 citation provenance graph block gate referenced by sourceBlockGateAtom",
  ],
  entryCriteria: {
    description:
      "P04-B07-A01 formalizes risk and trade-off research using sealed contradiction freshness artifacts",
    requiresBlockGatePass: true,
    contradictionFreshnessRecordRequired: true,
  },
};

export function getForgeP04B06BlockGate(): ForgeBlockGateDefinition {
  return FORGE_P04_B06_BLOCK_GATE_V1;
}

export function getForgeP04B06ToB07Handoff(): ResearcherContradictionFreshnessBlockHandoffContract {
  return FORGE_P04_B06_TO_B07_HANDOFF_V1;
}

export function validateResearcherContradictionFreshnessBlockHandoffContract(
  handoff: ResearcherContradictionFreshnessBlockHandoffContract,
  evidence: Pick<
    ResearcherContradictionFreshnessBlockGateEvidence,
  ...
  if (handoff.sealedArtifacts.harnessVersion !== FORGE_RESEARCHER_CONTRADICTION_FRESHNESS_VERSION) {
    issues.push(
      `handoff harnessVersion=${handoff.sealedArtifacts.harnessVersion} active=${FORGE_RESEARCHER_CONTRADICTION_FRESHNESS_VERSION}`,
    );
  }
  if (
    ...
    issues.push("handoff contradictionFreshnessCategories incomplete");
  }
  if (handoff.sealedArtifacts.sourceBlockGateAtom !== "P04-B05-A10") {
...
export function buildResearcherContradictionFreshnessBlockGateEvidence(
  atomSeals: ForgeBlockAtomSeal[],
  regressionPassed: boolean,
  guardPassed: boolean,
  probeCount: number,
  gitCommit?: string,
  blockId = FORGE_P04_B06_BLOCK_GATE_V1.blockId,
): ResearcherContradictionFreshnessBlockGateEvidence {
  const handoff = getForgeP04B06ToB07Handoff();
  const handoffValid = validateResearcherContradictionFreshnessBlockHandoffContract(handoff, {
    probeCount,
    regressionPassed,
    guardPassed,
  }).valid;
