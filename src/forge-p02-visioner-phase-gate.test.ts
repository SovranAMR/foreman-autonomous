...
describe("Forge Visioner Phase Gate Evidence — P02-B10-A06", () => {
  it("builds run record with disposition, criterion and aligned probe outcomes", () => {
    ...
    const telemetry = probeIds.map((probeId, index) => {
      const contractProbe = contract.probes.find(p => p.id === probeId)!;
      return buildVisionerPhaseGateProbeTelemetry(probeId, contractProbe.category, index, index * 0.5);
    });
  ...
  it("records evidence, telemetry and provenance for failure/recovery slice run", () => {
    ...
    assert.equal(record.provenance.sliceAtom, "P02-B10-A06");
    ...
    assert.equal(record.provenance.harnessVersion, FORGE_VISIONER_PHASE_GATE_VERSION);
    ...
    const structuredRecovery = record.evidence.find(
      e => e.probeId === "vpg.structured_phase_gate_recovery",
    );
    assert.ok(structuredRecovery);
  ...
  it("records evidence, telemetry and provenance for full visioner phase gate run", () => {
    const contract = getActiveVisionerPhaseGateContract();
