 ...
 * A07: unit, property and fuzz validation for web primary-source research contract and run records.
 */
...
export const FORGE_RESEARCHER_WEB_PRIMARY_SOURCE_VERSION = "1.0.0-a07";
...
// ─── Property and fuzz validation (P04-B03-A07) ─────────────────────────────

export interface ResearcherWebPrimarySourcePropertyViolation {
  propertyId: string;
  detail: string;
}

export interface ResearcherWebPrimarySourcePropertyResult {
  passed: number;
  failed: ResearcherWebPrimarySourcePropertyViolation[];
  total: number;
  allPassed: boolean;
}

export type ResearcherWebPrimarySourcePropertyCheck = {
  id: string;
  description: string;
  check: (contract: ResearcherWebPrimarySourceContract) => string | null;
};

const RESEARCHER_WEB_PRIMARY_SOURCE_STRUCTURAL_PROPERTIES: readonly ResearcherWebPrimarySourcePropertyCheck[] =
  [
    {
      id: "categories_complete",
      description: "All eight web primary-source categories are declared",
      check: contract => {
        for (const category of RESEARCHER_WEB_PRIMARY_SOURCE_CATEGORIES) {
    ...
    },
    {
      ...
      description:
        "summarizeResearcherWebPrimarySourceContractCoverage totals match listResearcherWebPrimarySourceContractProbeIds",
      check: contract => {
        const summary = summarizeResearcherWebPrimarySourceContractCoverage(contract);
        const ids = listResearcherWebPrimarySourceContractProbeIds(contract);
        if (summary.totalProbes !== ids.length) {
    ...
    {
      id: "probe_id_prefix",
      description: "Probe ids are namespaced with rwps. prefix",
      check: contract => {
        for (const probe of contract.probes) {
          if (!probe.id.startsWith("rwps.")) {
            return `${probe.id} missing rwps. prefix`;
          }
        }
        return null;
      },
    ...
    },
    {
      ...
      description:
        "Synthetic failure/recovery slice record passes validateResearcherWebPrimarySourceEvidenceRunRecord",
      check: contract => {
        const fixture = loadResearcherWebPrimarySourceBaseline();
...
export function runResearcherWebPrimarySourcePropertyChecks(
  contract: ResearcherWebPrimarySourceContract = getActiveResearcherWebPrimarySourceContract(),
): ResearcherWebPrimarySourcePropertyResult {
  const failed: ResearcherWebPrimarySourcePropertyViolation[] = [];
  for (const property of RESEARCHER_WEB_PRIMARY_SOURCE_STRUCTURAL_PROPERTIES) {
    const detail = property.check(contract);
    if (detail) failed.push({ propertyId: property.id, detail });
  }
  const total = RESEARCHER_WEB_PRIMARY_SOURCE_STRUCTURAL_PROPERTIES.length;
  return {
...
export type ResearcherWebPrimarySourceFuzzMutationKind =
  | "flip_expected"
  | "drop_probe"
  | "extra_probe"
  | "rename_probe"
...
export interface ResearcherWebPrimarySourceFuzzMutationCase {
  seed: number;
  kind: ResearcherWebPrimarySourceFuzzMutationKind;
  probeId?: string;
  category?: ResearcherWebPrimarySourceCategory;
}

export interface ResearcherWebPrimarySourceFuzzValidationCaseResult {
  mutation: ResearcherWebPrimarySourceFuzzMutationCase;
  valid: boolean;
  issueKinds: string[];
}

export interface ResearcherWebPrimarySourceFuzzValidationResult {
  seed: number;
  iterations: number;
  rejected: number;
  accepted: number;
  cases: ResearcherWebPrimarySourceFuzzValidationCaseResult[];
  allMutationsRejected: boolean;
}

/** Deterministic PRNG for reproducible fuzz cases (mulberry32). */
export function createResearcherWebPrimarySourceFuzzRng(seed: number): () => number {
  let state = seed >>> 0;
  return () => {
...
function cloneResearcherWebPrimarySourceBaseline(
  fixture: ResearcherWebPrimarySourceBaseline,
): ResearcherWebPrimarySourceBaseline {
  return {
    ...fixture,
...
function pickResearcherWebPrimarySourceFuzzTarget(
  fixture: ResearcherWebPrimarySourceBaseline,
  rng: () => number,
): {
  category: ResearcherWebPrimarySourceCategory;
  ...
  const category =
    RESEARCHER_WEB_PRIMARY_SOURCE_CATEGORIES[
      Math.floor(rng() * RESEARCHER_WEB_PRIMARY_SOURCE_CATEGORIES.length)
    ]!;
...
export function applyResearcherWebPrimarySourceFuzzMutation(
  fixture: ResearcherWebPrimarySourceBaseline,
  mutation: ResearcherWebPrimarySourceFuzzMutationCase,
): ResearcherWebPrimarySourceBaseline {
  const mutated = cloneResearcherWebPrimarySourceBaseline(fixture);
  const targetCategory = mutation.category ?? RESEARCHER_WEB_PRIMARY_SOURCE_CATEGORIES[0]!;
  ...
  switch (mutation.kind) {
    ...
    case "extra_probe":
      mutated.probes = [
        ...mutated.probes,
        {
          id: `rwps.fuzz.extra.${mutation.seed}`,
    ...
    case "flip_category": {
      const probeId = mutation.probeId ?? categoryEntries[0]!.id;
      const entry = mutated.probes.find(e => e.id === probeId) ?? categoryEntries[0]!;
      const other = RESEARCHER_WEB_PRIMARY_SOURCE_CATEGORIES.find(c => c !== entry.category)!;
...
export function generateResearcherWebPrimarySourceFuzzMutationCases(
  fixture: ResearcherWebPrimarySourceBaseline,
  seed: number,
  iterations: number,
): ResearcherWebPrimarySourceFuzzMutationCase[] {
  const rng = createResearcherWebPrimarySourceFuzzRng(seed);
  const kinds: ResearcherWebPrimarySourceFuzzMutationKind[] = [
    "flip_expected",
    "drop_probe",
    "extra_probe",
    "rename_probe",
  ...
  const cases: ResearcherWebPrimarySourceFuzzMutationCase[] = [];

  for (let i = 0; i < iterations; i++) {
    const kind = kinds[Math.floor(rng() * kinds.length)]!;
    const target = pickResearcherWebPrimarySourceFuzzTarget(fixture, rng);
    cases.push({
      seed: seed + i,
      kind,
...
/** Fuzz harness: mutated fixtures must fail contract validation (P04-B03-A07). */
export function runResearcherWebPrimarySourceFuzzValidation(
  fixture: ResearcherWebPrimarySourceBaseline,
  contract: ResearcherWebPrimarySourceContract = getActiveResearcherWebPrimarySourceContract(),
  seed = 42,
  iterations = 24,
): ResearcherWebPrimarySourceFuzzValidationResult {
  const cases = generateResearcherWebPrimarySourceFuzzMutationCases(fixture, seed, iterations);
  const results: ResearcherWebPrimarySourceFuzzValidationCaseResult[] = [];
  let rejected = 0;
  let accepted = 0;

  for (const mutation of cases) {
    const mutated = applyResearcherWebPrimarySourceFuzzMutation(fixture, mutation);
    const validation = validateResearcherWebPrimarySourceAgainstContract(mutated, contract);
...
export type ResearcherWebPrimarySourceRunRecordFuzzKind =
  | "drop_evidence"
  | "drop_telemetry"
  | "wrong_total"
  | "wrong_slice_atom"
...
export interface ResearcherWebPrimarySourceRunRecordFuzzCase {
  kind: ResearcherWebPrimarySourceRunRecordFuzzKind;
  probeId?: string;
}

export function applyResearcherWebPrimarySourceRunRecordFuzzMutation(
  record: ResearcherWebPrimarySourceRunRecord,
  mutation: ResearcherWebPrimarySourceRunRecordFuzzCase,
): ResearcherWebPrimarySourceRunRecord {
  const cloned: ResearcherWebPrimarySourceRunRecord = {
  ...
  switch (mutation.kind) {
    ...
    case "wrong_slice_atom":
      cloned.provenance = { ...cloned.provenance, sliceAtom: "P04-B03-A99" };
      break;
  ...
  cloned.summary = buildResearcherWebPrimarySourceRunRecord(
    cloned.provenance,
    cloned.evidence,
    cloned.telemetry,
  ).summary;
...
function resolveResearcherWebPrimarySourceRunRecordValidator(
  record: ResearcherWebPrimarySourceRunRecord,
): (
  record: ResearcherWebPrimarySourceRunRecord,
  contract: ResearcherWebPrimarySourceContract,
) => ResearcherWebPrimarySourceRunValidationResult {
  return record.provenance.sliceAtom === "P04-B03-A06"
    ? validateResearcherWebPrimarySourceEvidenceRunRecord
    : validateResearcherWebPrimarySourceRunRecord;
}

/** Fuzz harness: tampered run records must fail validation deterministically (P04-B03-A07). */
export function runResearcherWebPrimarySourceRunRecordFuzzValidation(
  record: ResearcherWebPrimarySourceRunRecord,
  contract: ResearcherWebPrimarySourceContract = getActiveResearcherWebPrimarySourceContract(),
): { validBaseline: boolean; mutationsRejected: number; mutationsAccepted: number } {
  const validate = resolveResearcherWebPrimarySourceRunRecordValidator(record);
  const baseline = validate(record, contract);
  const probeId = record.evidence[0]?.probeId;
  const mutations: ResearcherWebPrimarySourceRunRecordFuzzCase[] = [
  ...
  for (const mutation of mutations) {
    const mutated = applyResearcherWebPrimarySourceRunRecordFuzzMutation(record, mutation);
...
export interface ResearcherWebPrimarySourcePropertyFuzzSliceResult {
  atom: "P04-B03-A07";
  propertyChecksPassed: boolean;
  contractFuzzRejected: boolean;
  runRecordFuzzRejected: boolean;
  propertyResult: ResearcherWebPrimarySourcePropertyResult;
  contractFuzz: ResearcherWebPrimarySourceFuzzValidationResult;
  runRecordFuzz: {
 ...
 */
export function runResearcherWebPrimarySourcePropertyFuzzSlice(
  fixture: ResearcherWebPrimarySourceBaseline = loadResearcherWebPrimarySourceBaseline(),
): ResearcherWebPrimarySourcePropertyFuzzSliceResult {
  const contract = getActiveResearcherWebPrimarySourceContract();
  const propertyResult = runResearcherWebPrimarySourcePropertyChecks(contract);
  const contractFuzz = runResearcherWebPrimarySourceFuzzValidation(fixture, contract);
  const record = runResearcherWebPrimarySourceFailureRecoverySliceWithRecord(fixture);
  const runRecordFuzz = runResearcherWebPrimarySourceRunRecordFuzzValidation(record, contract);
...
// ─── Probe regression detection (P04-B03-A08) ────────────────────────────────

export interface ResearcherWebPrimarySourceProbeRegressionReport {
...
/**
 * Compare web primary-source run records and detect probe alignment regressions.
 * A regression = probe aligned in prior run but misaligned in current run.
 */
export function detectResearcherWebPrimarySourceProbeRegression(
  prior: ResearcherWebPrimarySourceRunRecord,
  current: ResearcherWebPrimarySourceRunRecord,
): ResearcherWebPrimarySourceProbeRegressionReport {
...
// ─── Guard controls (P04-B03-A09 foundation, used by A08 regression gate) ─────

export interface ForgeResearcherWebPrimarySourceGuardControls {
  atom: string;
  adversarial: {
...
export interface ResearcherWebPrimarySourceGuardCheckIssue {
  domain: "adversarial" | "performance" | "cost" | "safety";
  code: string;
  detail: string;
}

export interface ResearcherWebPrimarySourceGuardCheckResult {
  passed: boolean;
  issues: ResearcherWebPrimarySourceGuardCheckIssue[];
  metrics: {
...
export interface ResearcherWebPrimarySourceAdversarialGuardScenario {
  id: string;
  description: string;
  build: (record: ResearcherWebPrimarySourceRunRecord) => ResearcherWebPrimarySourceRunRecord;
  expectRejected: true;
}

export const FORGE_RESEARCHER_WEB_PRIMARY_SOURCE_GUARD_CONTROLS_V1: ForgeResearcherWebPrimarySourceGuardControls =
  {
    atom: "P04-B03-A09",
    adversarial: {
      rejectTamperedRecords: true,
...
export function getForgeResearcherWebPrimarySourceGuardControls(): ForgeResearcherWebPrimarySourceGuardControls {
  return FORGE_RESEARCHER_WEB_PRIMARY_SOURCE_GUARD_CONTROLS_V1;
}

function parseResearcherWebPrimarySourceIsoDurationMs(
  startedAt: string,
  completedAt: string,
): number {
...
export function summarizeResearcherWebPrimarySourceTelemetry(
  telemetry: ResearcherWebPrimarySourceProbeTelemetry[],
): {
  suiteDurationMs: number;
  maxProbeDurationMs: number;
...
export function detectResearcherWebPrimarySourceEvidenceSummaryMismatch(
  record: ResearcherWebPrimarySourceRunRecord,
): string | null {
  let alignedCount = 0;
  for (const item of record.evidence) {
...
export function detectResearcherWebPrimarySourceFalseAlignment(
  record: ResearcherWebPrimarySourceRunRecord,
): string[] {
...
export function validateResearcherWebPrimarySourceSafety(
  record: ResearcherWebPrimarySourceRunRecord,
  controls: ForgeResearcherWebPrimarySourceGuardControls = getForgeResearcherWebPrimarySourceGuardControls(),
): ResearcherWebPrimarySourceGuardCheckIssue[] {
  const issues: ResearcherWebPrimarySourceGuardCheckIssue[] = [];
  for (const item of record.evidence) {
...
export function validateResearcherWebPrimarySourcePerformance(
  record: ResearcherWebPrimarySourceRunRecord,
  controls: ForgeResearcherWebPrimarySourceGuardControls = getForgeResearcherWebPrimarySourceGuardControls(),
): ResearcherWebPrimarySourceGuardCheckIssue[] {
  const issues: ResearcherWebPrimarySourceGuardCheckIssue[] = [];
  const { suiteDurationMs, maxProbeDurationMs } = summarizeResearcherWebPrimarySourceTelemetry(
    record.telemetry,
  );
  const wallClockMs = parseResearcherWebPrimarySourceIsoDurationMs(
...
export function validateResearcherWebPrimarySourceCost(
  totalCostUsd: number,
  llmCalls: number,
  controls: ForgeResearcherWebPrimarySourceGuardControls = getForgeResearcherWebPrimarySourceGuardControls(),
): ResearcherWebPrimarySourceGuardCheckIssue[] {
...
export function buildResearcherWebPrimarySourceAdversarialGuardScenarios(): ResearcherWebPrimarySourceAdversarialGuardScenario[] {
  return [
    {
...
export function runResearcherWebPrimarySourceAdversarialGuardChecks(
  fixtureRecord: ResearcherWebPrimarySourceRunRecord,
  contract: ResearcherWebPrimarySourceContract = getActiveResearcherWebPrimarySourceContract(),
  ...
  for (const scenario of scenarios) {
    ...
    const validation = validateResearcherWebPrimarySourceRunRecord(tampered, contract);
    const falseAlignment = detectResearcherWebPrimarySourceFalseAlignment(tampered);
    const summaryMismatch = detectResearcherWebPrimarySourceEvidenceSummaryMismatch(tampered);
...
export function validateForgeResearcherWebPrimarySourceGuard(
  record: ResearcherWebPrimarySourceRunRecord,
  options: {
    totalCostUsd?: number;
    llmCalls?: number;
    contract?: ResearcherWebPrimarySourceContract;
    controls?: ForgeResearcherWebPrimarySourceGuardControls;
  } = {},
): ResearcherWebPrimarySourceGuardCheckResult {
  const controls = options.controls ?? getForgeResearcherWebPrimarySourceGuardControls();
  const contract = options.contract ?? getActiveResearcherWebPrimarySourceContract();
  const totalCostUsd = options.totalCostUsd ?? 0;
  const llmCalls = options.llmCalls ?? 0;
  const issues: ResearcherWebPrimarySourceGuardCheckIssue[] = [];

  issues.push(...validateResearcherWebPrimarySourcePerformance(record, controls));
  issues.push(...validateResearcherWebPrimarySourceCost(totalCostUsd, llmCalls, controls));
  issues.push(...validateResearcherWebPrimarySourceSafety(record, controls));

  const falseAlignment = detectResearcherWebPrimarySourceFalseAlignment(record);
  ...
  const summaryMismatch = detectResearcherWebPrimarySourceEvidenceSummaryMismatch(record);
  if (summaryMismatch) {
    issues.push({
      domain: "adversarial",
  ...
  const telemetrySummary = summarizeResearcherWebPrimarySourceTelemetry(record.telemetry);
  const wallClockMs = parseResearcherWebPrimarySourceIsoDurationMs(
    record.provenance.startedAt,
    record.provenance.completedAt,
  );
...
export interface ResearcherWebPrimarySourceForgeRegressionResult {
  atom: "P04-B03-A08";
  passed: boolean;
  productionSlice: ResearcherWebPrimarySourceProductionSliceResult;
  propertyFuzzSlice: ResearcherWebPrimarySourcePropertyFuzzSliceResult;
  record: ResearcherWebPrimarySourceRunRecord;
  recordValid: boolean;
  priorRecordValid: boolean;
  validationIssues: string[];
  priorValidationIssues: string[];
  probeRegression: ResearcherWebPrimarySourceProbeRegressionReport | null;
  guard: ResearcherWebPrimarySourceGuardCheckResult;
  detail: string;
}

/**
 * Execute web primary-source probes, validate production slice + run record, property/fuzz gates,
 * and optionally detect regression vs prior run. Forge pipeline integration gate (P04-B03-A08).
 */
export function runResearcherWebPrimarySourceForgeRegression(
  ...
  const record = runResearcherWebPrimarySourceProbesWithRecord(fixture);
  ...
  if (priorRecord) {
    const priorValidation = validateResearcherWebPrimarySourceRunRecord(priorRecord, contract);
  ...
  const probeRegression = priorRecord
    ? detectResearcherWebPrimarySourceProbeRegression(priorRecord, record)
    : null;
