...
// ─── Probe regression detection (P04-B10-A08) ────────────────────────────────

export interface ResearcherPhaseGateProbeRegressionReport {
...
/**
 * Compare researcher phase gate run records and detect probe alignment regressions.
 * A regression = probe aligned in prior run but misaligned in current run.
 */
export function detectResearcherPhaseGateProbeRegression(
  prior: ResearcherPhaseGateRunRecord,
  current: ResearcherPhaseGateRunRecord,
): ResearcherPhaseGateProbeRegressionReport {
...
// ─── Guard controls (P04-B10-A09 foundation, used by A08 regression gate) ────

export interface ForgeResearcherPhaseGateGuardControls {
  atom: string;
  adversarial: {
...
export interface ResearcherPhaseGateGuardCheckIssue {
  domain: "adversarial" | "performance" | "cost" | "safety";
  code: string;
  detail: string;
}

export interface ResearcherPhaseGateGuardCheckResult {
  passed: boolean;
  issues: ResearcherPhaseGateGuardCheckIssue[];
  metrics: {
...
export interface ResearcherPhaseGateAdversarialGuardScenario {
  id: string;
  description: string;
  build: (record: ResearcherPhaseGateRunRecord) => ResearcherPhaseGateRunRecord;
  expectRejected: true;
}

export const FORGE_RESEARCHER_PHASE_GATE_GUARD_CONTROLS_V1: ForgeResearcherPhaseGateGuardControls = {
  atom: "P04-B10-A09",
  adversarial: {
    rejectTamperedRecords: true,
    rejectFalseAlignment: true,
...
export function getForgeResearcherPhaseGateGuardControls(): ForgeResearcherPhaseGateGuardControls {
  return FORGE_RESEARCHER_PHASE_GATE_GUARD_CONTROLS_V1;
}

function parseResearcherPhaseGateIsoDurationMs(startedAt: string, completedAt: string): number {
...
export function summarizeResearcherPhaseGateTelemetry(
  telemetry: ResearcherPhaseGateProbeTelemetry[],
): {
  suiteDurationMs: number;
  maxProbeDurationMs: number;
...
export function detectResearcherPhaseGateEvidenceSummaryMismatch(
  record: ResearcherPhaseGateRunRecord,
): string | null {
  let alignedCount = 0;
  for (const item of record.evidence) {
...
export function detectResearcherPhaseGateFalseAlignment(
  record: ResearcherPhaseGateRunRecord,
): string[] {
...
export function validateResearcherPhaseGateSafety(
  record: ResearcherPhaseGateRunRecord,
  controls: ForgeResearcherPhaseGateGuardControls = getForgeResearcherPhaseGateGuardControls(),
): ResearcherPhaseGateGuardCheckIssue[] {
  const issues: ResearcherPhaseGateGuardCheckIssue[] = [];
  for (const item of record.evidence) {
...
export function validateResearcherPhaseGatePerformance(
  record: ResearcherPhaseGateRunRecord,
  controls: ForgeResearcherPhaseGateGuardControls = getForgeResearcherPhaseGateGuardControls(),
): ResearcherPhaseGateGuardCheckIssue[] {
  const issues: ResearcherPhaseGateGuardCheckIssue[] = [];
  const { suiteDurationMs, maxProbeDurationMs } = summarizeResearcherPhaseGateTelemetry(
    record.telemetry,
  );
  const wallClockMs = parseResearcherPhaseGateIsoDurationMs(
...
export function validateResearcherPhaseGateCost(
  totalCostUsd: number,
  llmCalls: number,
  controls: ForgeResearcherPhaseGateGuardControls = getForgeResearcherPhaseGateGuardControls(),
): ResearcherPhaseGateGuardCheckIssue[] {
...
export function buildResearcherPhaseGateAdversarialGuardScenarios(): ResearcherPhaseGateAdversarialGuardScenario[] {
  return [
    {
...
export function runResearcherPhaseGateAdversarialGuardChecks(
  fixtureRecord: ResearcherPhaseGateRunRecord,
  contract: ResearcherPhaseGateContract = getActiveResearcherPhaseGateContract(),
  ...
  for (const scenario of scenarios) {
    ...
    const validate = resolveResearcherPhaseGateRunRecordValidator(tampered);
    const validation = validate(tampered, contract);
    const falseAlignment = detectResearcherPhaseGateFalseAlignment(tampered);
    const summaryMismatch = detectResearcherPhaseGateEvidenceSummaryMismatch(tampered);
...
export function validateForgeResearcherPhaseGateGuard(
  record: ResearcherPhaseGateRunRecord,
  options: {
    totalCostUsd?: number;
    llmCalls?: number;
    contract?: ResearcherPhaseGateContract;
    controls?: ForgeResearcherPhaseGateGuardControls;
  } = {},
): ResearcherPhaseGateGuardCheckResult {
  const controls = options.controls ?? getForgeResearcherPhaseGateGuardControls();
  const contract = options.contract ?? getActiveResearcherPhaseGateContract();
  const totalCostUsd = options.totalCostUsd ?? 0;
  const llmCalls = options.llmCalls ?? 0;
  const issues: ResearcherPhaseGateGuardCheckIssue[] = [];

  issues.push(...validateResearcherPhaseGatePerformance(record, controls));
  issues.push(...validateResearcherPhaseGateCost(totalCostUsd, llmCalls, controls));
  issues.push(...validateResearcherPhaseGateSafety(record, controls));

  const falseAlignment = detectResearcherPhaseGateFalseAlignment(record);
  ...
  const summaryMismatch = detectResearcherPhaseGateEvidenceSummaryMismatch(record);
  if (summaryMismatch) {
    issues.push({
      domain: "adversarial",
  ...
  const telemetrySummary = summarizeResearcherPhaseGateTelemetry(record.telemetry);
  const wallClockMs = parseResearcherPhaseGateIsoDurationMs(
    record.provenance.startedAt,
    record.provenance.completedAt,
  );
