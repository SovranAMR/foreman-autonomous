...
import {
  buildVisionerGroundingAdversarialGuardScenarios,
  ...
  detectVisionerGroundingEvidenceSummaryMismatch,
  detectVisionerGroundingFalseAlignment,
  getActiveVisionerGroundingContract,
  getForgeVisionerGroundingGuardControls,
  listVisionerGroundingContractProbeIds,
  runVisionerGroundingAdversarialGuardChecks,
  validateForgeVisionerGroundingGuard,
  validateVisionerGroundingCost,
  validateVisionerGroundingPerformance,
  validateVisionerGroundingSafety,
} from "./forge-p02-visioner-grounding.js";

describe("Forge Visioner Grounding Guard — P02-B04-A09 adversarial", () => {
  ...
  it("detects false alignment and summary/evidence mismatch", () => {
    const falsePassEvidence = buildVisionerGroundingProbeEvidence(
      "vgrd.version_tagged",
    ...
    const contract = getActiveVisionerGroundingContract();
    const falsePassRecord = buildVisionerGroundingRunRecord(
      buildVisionerGroundingProvenance(
        "adv-false-pass",
        fixture,
        contract,
      ...
      [falsePassEvidence],
      [buildVisionerGroundingProbeTelemetry("vgrd.version_tagged", "grounding_versioning", 0, 1)],
    );
    assert.ok(detectVisionerGroundingFalseAlignment(falsePassRecord).length > 0);

    const summaryEvidence = buildVisionerGroundingProbeEvidence(
      "vgrd.version_tagged",
      "grounding_versioning",
      "PASS",
      "FAIL",
    ...
    const summaryRecord = buildVisionerGroundingRunRecord(
      buildVisionerGroundingProvenance(
        "adv-summary",
        fixture,
        contract,
    ...
    assert.ok(detectVisionerGroundingEvidenceSummaryMismatch(mismatchedSummary));
  });

  it("buildVisionerGroundingAdversarialGuardScenarios cover false PASS attack vectors", () => {
...
describe("Forge Visioner Grounding Guard — P02-B04-A09 performance, cost, safety", () => {
  it("passes performance and zero-cost guard on canonical visioner grounding run", () => {
    const record = runVisionerGroundingProbesWithRecord();
    const contract = getActiveVisionerGroundingContract();
    ...
    assert.ok(
      guard.metrics.maxProbeDurationMs <
        getForgeVisionerGroundingGuardControls().performance.maxProbeDurationMs,
    );
  ...
  it("flags cost and performance budget violations", () => {
    ...
    const telemetry = probeIds.map((id, index) => {
      const probe = contract.probes.find(p => p.id === id)!;
      return buildVisionerGroundingProbeTelemetry(id, probe.category, index, 10_000);
    });
    ...
    const perfIssues = validateVisionerGroundingPerformance(record);
    assert.ok(perfIssues.some(i => i.domain === "performance"));

    const costIssues = validateVisionerGroundingCost(0.05, 2);
  ...
  it("flags forbidden secret patterns in evidence detail", () => {
    const fixture = loadVisionerGroundingBaseline();
    const contract = getActiveVisionerGroundingContract();
    const evidence = buildVisionerGroundingProbeEvidence(
      "vgrd.version_tagged",
      "grounding_versioning",
      "PASS",
      "PASS",
    ...
    const safetyIssues = validateVisionerGroundingSafety(record);
    assert.ok(safetyIssues.some(i => i.code === "forbidden_pattern"));
  });
});

describe("Forge Visioner Grounding Guard — P02-B04-A09 integration", () => {
  it("runForgeVisionerGroundingRegressionGate includes guard PASS in detail", () => {
  ...
  it("orchestrator verifyForgeVisionerGroundingGuard emits visioner_grounding_guard verification", async () => {
    const root = mkdtempSync(join(tmpdir(), "forge-visioner-grounding-guard-orch-"));
    const engine = {
    ...
    const result = await orchestrator.verifyForgeVisionerGroundingGuard();
    const verification = events.find(
      event => event.type === "verification" && event.phase === "visioner_grounding_guard",
    );
