...
// ─── Probe regression detection (P01-B09-A08) ────────────────────────────────

export interface OrchestratorSeamProbeRegressionReport {
...
/**
 * Compare orchestrator seam run records and detect probe alignment regressions.
 * A regression = probe aligned in prior run but misaligned in current run.
 */
export function detectOrchestratorSeamProbeRegression(
  prior: OrchestratorSeamRunRecord,
  current: OrchestratorSeamRunRecord,
): OrchestratorSeamProbeRegressionReport {
...
// ─── Guard controls (P01-B09-A09 foundation, used by A08 regression gate) ────

export interface ForgeOrchestratorSeamGuardControls {
  atom: string;
  adversarial: {
...
export interface OrchestratorSeamGuardCheckIssue {
  domain: "adversarial" | "performance" | "cost" | "safety";
  code: string;
  detail: string;
}

export interface OrchestratorSeamGuardCheckResult {
  passed: boolean;
  issues: OrchestratorSeamGuardCheckIssue[];
  metrics: {
...
export interface OrchestratorSeamAdversarialGuardScenario {
  id: string;
  description: string;
  build: (record: OrchestratorSeamRunRecord) => OrchestratorSeamRunRecord;
  expectRejected: true;
}

export const FORGE_ORCHESTRATOR_SEAM_GUARD_CONTROLS_V1: ForgeOrchestratorSeamGuardControls = {
  atom: "P01-B09-A09",
  adversarial: {
    rejectTamperedRecords: true,
    rejectFalseAlignment: true,
...
export function getForgeOrchestratorSeamGuardControls(): ForgeOrchestratorSeamGuardControls {
  return FORGE_ORCHESTRATOR_SEAM_GUARD_CONTROLS_V1;
}

function parseOrchestratorSeamIsoDurationMs(startedAt: string, completedAt: string): number {
...
export function summarizeOrchestratorSeamTelemetry(telemetry: OrchestratorSeamProbeTelemetry[]): {
  suiteDurationMs: number;
  maxProbeDurationMs: number;
} {
...
export function detectOrchestratorSeamEvidenceSummaryMismatch(
  record: OrchestratorSeamRunRecord,
): string | null {
  let alignedCount = 0;
  for (const item of record.evidence) {
...
export function detectOrchestratorSeamFalseAlignment(record: OrchestratorSeamRunRecord): string[] {
...
export function validateOrchestratorSeamSafety(
  record: OrchestratorSeamRunRecord,
  controls: ForgeOrchestratorSeamGuardControls = getForgeOrchestratorSeamGuardControls(),
): OrchestratorSeamGuardCheckIssue[] {
  const issues: OrchestratorSeamGuardCheckIssue[] = [];
  for (const item of record.evidence) {
...
export function validateOrchestratorSeamPerformance(
  record: OrchestratorSeamRunRecord,
  controls: ForgeOrchestratorSeamGuardControls = getForgeOrchestratorSeamGuardControls(),
): OrchestratorSeamGuardCheckIssue[] {
  const issues: OrchestratorSeamGuardCheckIssue[] = [];
  const { suiteDurationMs, maxProbeDurationMs } = summarizeOrchestratorSeamTelemetry(record.telemetry);
  const wallClockMs = parseOrchestratorSeamIsoDurationMs(
...
export function validateOrchestratorSeamCost(
  totalCostUsd: number,
  llmCalls: number,
  controls: ForgeOrchestratorSeamGuardControls = getForgeOrchestratorSeamGuardControls(),
): OrchestratorSeamGuardCheckIssue[] {
...
export function buildOrchestratorSeamAdversarialGuardScenarios(): OrchestratorSeamAdversarialGuardScenario[] {
  return [
    {
...
export function runOrchestratorSeamAdversarialGuardChecks(
  fixtureRecord: OrchestratorSeamRunRecord,
  contract: OrchestratorSeamContract = getActiveOrchestratorSeamContract(),
  ...
  for (const scenario of scenarios) {
    ...
    const validation = validateOrchestratorSeamRunRecord(tampered, contract);
    const falseAlignment = detectOrchestratorSeamFalseAlignment(tampered);
    const summaryMismatch = detectOrchestratorSeamEvidenceSummaryMismatch(tampered);
...
export function validateForgeOrchestratorSeamGuard(
  record: OrchestratorSeamRunRecord,
  options: {
    totalCostUsd?: number;
    llmCalls?: number;
    contract?: OrchestratorSeamContract;
    controls?: ForgeOrchestratorSeamGuardControls;
  } = {},
): OrchestratorSeamGuardCheckResult {
  const controls = options.controls ?? getForgeOrchestratorSeamGuardControls();
  const contract = options.contract ?? getActiveOrchestratorSeamContract();
  const totalCostUsd = options.totalCostUsd ?? 0;
  const llmCalls = options.llmCalls ?? 0;
  const issues: OrchestratorSeamGuardCheckIssue[] = [];

  issues.push(...validateOrchestratorSeamPerformance(record, controls));
  issues.push(...validateOrchestratorSeamCost(totalCostUsd, llmCalls, controls));
  issues.push(...validateOrchestratorSeamSafety(record, controls));

  const falseAlignment = detectOrchestratorSeamFalseAlignment(record);
  ...
  const summaryMismatch = detectOrchestratorSeamEvidenceSummaryMismatch(record);
  if (summaryMismatch) {
    issues.push({
      domain: "adversarial",
  ...
  const adversarial = runOrchestratorSeamAdversarialGuardChecks(record, contract);
  ...
  const telemetrySummary = summarizeOrchestratorSeamTelemetry(record.telemetry);
  const wallClockMs = parseOrchestratorSeamIsoDurationMs(
    record.provenance.startedAt,
    record.provenance.completedAt,
  );
