...
export interface ForgeBenchmarkEvalBlockGateResult {
  passed: boolean;
  evidence: import("./forge-benchmark-eval-harness.js").BenchmarkEvalBlockGateEvidence;
  handoff: import("./forge-benchmark-eval-harness.js").BenchmarkEvalBlockHandoffContract;
  regression: ForgeBenchmarkEvalRegressionResult;
  atomSeals: ForgeBlockAtomSeal[];
  detail: string;
}

function sealBenchmarkEvalBlockAtom(
  atomId: string,
  capability: string,
  passed: boolean,
  detail: string,
...
/**
 * Seal P01-B06 block gate: validate A01–A09 deliverables, regression, guard, and B07 handoff (P01-B06-A10).
 */
export function runForgeBenchmarkEvalBlockGate(): ForgeBenchmarkEvalBlockGateResult {
  const blockGate = getForgeP01B06BlockGate();
  const handoff = getForgeP01B06ToB07Handoff();
  const contract = getActiveBenchmarkEvalContract();
  const fixture = loadBenchmarkEvalHarnessFixture();
  const atomSeals: ForgeBlockAtomSeal[] = [];

  const fixtureValidation = validateBenchmarkEvalHarnessFixtureAgainstContract(fixture, contract);
  atomSeals.push(
    sealBenchmarkEvalBlockAtom(
      "P01-B06-A01",
      "benchmark_fixture",
      fixtureValidation.valid && fixture.version === handoff.sealedArtifacts.fixtureVersion,
      fixtureValidation.valid
        ? `fixture v${fixture.version} aligned (${summarizeBenchmarkEvalContractCoverage(contract).totalProbes} probes)`
  ...
  atomSeals.push(
    sealBenchmarkEvalBlockAtom(
      ...
      `${coverage.totalProbes} probes across ${BENCHMARK_EVAL_CATEGORIES.length} categories`,
    ),
  );

  const productionSlice = runBenchmarkEvalProductionSlice(fixture);
  atomSeals.push(
    sealBenchmarkEvalBlockAtom(
      "P01-B06-A03",
      "probe_matrix",
  ...
  const boundarySlice = runBenchmarkEvalBoundarySlice(fixture);
  const dispositionOk =
  ...
  const failureRecoverySlice = runBenchmarkEvalFailureRecoverySlice(fixture);
  const nogoProbes = listBenchmarkEvalProbesByDisposition("nogo", contract);
  atomSeals.push(
    sealBenchmarkEvalBlockAtom(
      "P01-B06-A05",
  ...
  const regression = runForgeBenchmarkEvalRegressionGate();
  const recordValidation = validateBenchmarkEvalRunRecord(regression.record);
  const evidenceOk =
  ...
  const properties = runBenchmarkEvalPropertyChecks(contract);
  const contractFuzz = runBenchmarkEvalFuzzValidation(fixture, contract);
  const runFuzz = runBenchmarkEvalRunRecordFuzzValidation(regression.record, contract);
  ...
  const handoffValidation = validateBenchmarkEvalBlockHandoffContract(handoff, {
  ...
  const evidence = buildBenchmarkEvalBlockGateEvidence(
    atomSeals,
    regression.passed,
    regression.guard.passed,
