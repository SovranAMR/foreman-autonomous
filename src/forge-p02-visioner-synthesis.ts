...
export const FORGE_VISIONER_SYNTHESIS_VERSION = "1.0.0-a10";
...
// ─── Probe regression detection (P02-B03-A08) ────────────────────────────────

export interface VisionerSynthesisProbeRegressionReport {
...
/**
 * Compare visioner synthesis run records and detect probe alignment regressions.
 * A regression = probe aligned in prior run but misaligned in current run.
 */
export function detectVisionerSynthesisProbeRegression(
  prior: VisionerSynthesisRunRecord,
  current: VisionerSynthesisRunRecord,
): VisionerSynthesisProbeRegressionReport {
...
// ─── Guard controls (P02-B03-A09 foundation, used by A08 regression gate) ───

export interface ForgeVisionerSynthesisGuardControls {
  atom: string;
  adversarial: {
...
export interface VisionerSynthesisGuardCheckIssue {
  domain: "adversarial" | "performance" | "cost" | "safety";
  code: string;
  detail: string;
}

export interface VisionerSynthesisGuardCheckResult {
  passed: boolean;
  issues: VisionerSynthesisGuardCheckIssue[];
  metrics: {
...
export interface VisionerSynthesisAdversarialGuardScenario {
  id: string;
  description: string;
  build: (record: VisionerSynthesisRunRecord) => VisionerSynthesisRunRecord;
  expectRejected: true;
}

export const FORGE_VISIONER_SYNTHESIS_GUARD_CONTROLS_V1: ForgeVisionerSynthesisGuardControls = {
  atom: "P02-B03-A09",
  adversarial: {
    rejectTamperedRecords: true,
    rejectFalseAlignment: true,
...
export function getForgeVisionerSynthesisGuardControls(): ForgeVisionerSynthesisGuardControls {
  return FORGE_VISIONER_SYNTHESIS_GUARD_CONTROLS_V1;
}

function parseVisionerSynthesisIsoDurationMs(startedAt: string, completedAt: string): number {
...
export function summarizeVisionerSynthesisTelemetry(telemetry: VisionerSynthesisProbeTelemetry[]): {
  suiteDurationMs: number;
  maxProbeDurationMs: number;
} {
...
export function detectVisionerSynthesisEvidenceSummaryMismatch(
  record: VisionerSynthesisRunRecord,
): string | null {
  let alignedCount = 0;
  for (const item of record.evidence) {
...
export function detectVisionerSynthesisFalseAlignment(record: VisionerSynthesisRunRecord): string[] {
...
export function validateVisionerSynthesisSafety(
  record: VisionerSynthesisRunRecord,
  controls: ForgeVisionerSynthesisGuardControls = getForgeVisionerSynthesisGuardControls(),
): VisionerSynthesisGuardCheckIssue[] {
  const issues: VisionerSynthesisGuardCheckIssue[] = [];
  for (const item of record.evidence) {
...
export function validateVisionerSynthesisPerformance(
  record: VisionerSynthesisRunRecord,
  controls: ForgeVisionerSynthesisGuardControls = getForgeVisionerSynthesisGuardControls(),
): VisionerSynthesisGuardCheckIssue[] {
  const issues: VisionerSynthesisGuardCheckIssue[] = [];
  const { suiteDurationMs, maxProbeDurationMs } = summarizeVisionerSynthesisTelemetry(record.telemetry);
  const wallClockMs = parseVisionerSynthesisIsoDurationMs(
...
export function validateVisionerSynthesisCost(
  totalCostUsd: number,
  llmCalls: number,
  controls: ForgeVisionerSynthesisGuardControls = getForgeVisionerSynthesisGuardControls(),
): VisionerSynthesisGuardCheckIssue[] {
...
export function buildVisionerSynthesisAdversarialGuardScenarios(): VisionerSynthesisAdversarialGuardScenario[] {
  return [
    {
...
export function runVisionerSynthesisAdversarialGuardChecks(
  fixtureRecord: VisionerSynthesisRunRecord,
  contract: VisionerSynthesisContract = getActiveVisionerSynthesisContract(),
  ...
  for (const scenario of scenarios) {
    ...
    const validation = validateVisionerSynthesisRunRecord(tampered, contract);
    const falseAlignment = detectVisionerSynthesisFalseAlignment(tampered);
    const summaryMismatch = detectVisionerSynthesisEvidenceSummaryMismatch(tampered);
...
export function validateForgeVisionerSynthesisGuard(
  record: VisionerSynthesisRunRecord,
  options: {
    totalCostUsd?: number;
    llmCalls?: number;
    contract?: VisionerSynthesisContract;
    controls?: ForgeVisionerSynthesisGuardControls;
  } = {},
): VisionerSynthesisGuardCheckResult {
  const controls = options.controls ?? getForgeVisionerSynthesisGuardControls();
  const contract = options.contract ?? getActiveVisionerSynthesisContract();
  const totalCostUsd = options.totalCostUsd ?? 0;
  const llmCalls = options.llmCalls ?? 0;
  const issues: VisionerSynthesisGuardCheckIssue[] = [];

  issues.push(...validateVisionerSynthesisPerformance(record, controls));
  issues.push(...validateVisionerSynthesisCost(totalCostUsd, llmCalls, controls));
  issues.push(...validateVisionerSynthesisSafety(record, controls));

  const falseAlignment = detectVisionerSynthesisFalseAlignment(record);
  ...
  const summaryMismatch = detectVisionerSynthesisEvidenceSummaryMismatch(record);
  if (summaryMismatch) {
    issues.push({
      domain: "adversarial",
  ...
  const telemetrySummary = summarizeVisionerSynthesisTelemetry(record.telemetry);
  const wallClockMs = parseVisionerSynthesisIsoDurationMs(
    record.provenance.startedAt,
    record.provenance.completedAt,
  );
...
// ─── Block gate and handoff (P02-B03-A10) ─────────────────────────────────────

export interface VisionerSynthesisBlockGateEvidence {
  blockId: string;
  atom: string;
  sealedAt: string;
...
export interface VisionerSynthesisBlockHandoffContract {
  version: string;
  atom: string;
  sourceBlock: {
    blockId: string;
  ...
  sealedArtifacts: {
    ...
    harnessVersion: string;
    probeCount: number;
    visionerSynthesisCategories: readonly VisionerSynthesisCategory[];
    sourceVisionerConstraintBlockGateAtom: string;
  };
  prerequisites: readonly string[];
  entryCriteria: {
    description: string;
    requiresBlockGatePass: true;
    visionerSynthesisRecordRequired: true;
  };
}

export const FORGE_P02_B03_BLOCK_GATE_V1: ForgeBlockGateDefinition = {
  version: "1.0.0",
  atom: "P02-B03-A10",
  blockId: "P02-B03",
  ...
  checks: [
    { id: "fixture_contract_alignment", atomId: "P02-B03-A01", description: "Visioner synthesis baseline aligns with typed contract and P02-B02 block gate handoff" },
    { id: "typed_contract_coverage", atomId: "P02-B03-A02", description: "Contract declares measurable probes for all visioner synthesis categories" },
    { id: "probe_matrix_aligned", atomId: "P02-B03-A03", description: "Visioner synthesis probe matrix executes with zero unexpected mismatches" },
    { id: "boundary_disposition_coverage", atomId: "P02-B03-A04", description: "Contract covers observed, gap, failure, recovery and NO-GO dispositions" },
    { id: "failure_recovery_nogo", atomId: "P02-B03-A05", description: "Failure, recovery and NO-GO probes are declared and exercised" },
    { id: "evidence_telemetry_provenance", atomId: "P02-B03-A06", description: "Run record carries evidence, telemetry and provenance" },
    { id: "property_and_fuzz", atomId: "P02-B03-A07", description: "Structural property and fuzz validation reject tampered inputs" },
    { id: "regression_gate", atomId: "P02-B03-A08", description: "Regression gate passes on canonical visioner synthesis matrix" },
    { id: "guard_controls", atomId: "P02-B03-A09", description: "Adversarial, performance, cost and safety guard controls pass" },
    { id: "block_gate_sealed", atomId: "P02-B03-A10", description: "Block gate evidence sealed with valid B04 handoff contract" },
  ] satisfies readonly ForgeBlockGateCheck[],
};

export const FORGE_P02_B03_TO_B04_HANDOFF_V1: VisionerSynthesisBlockHandoffContract = {
  version: "1.0.0",
  atom: "P02-B03-A10",
  sourceBlock: {
    blockId: "P02-B03",
    title: "Ürün vizyonu sentezi",
    completedAtoms: FORGE_P02_B03_BLOCK_GATE_V1.requiredAtomIds,
  },
  targetBlock: {
    blockId: "P02-B04",
    title: "Repo ve kullanıcı bağlamı grounding",
    entryAtom: "P02-B04-A01",
  },
  sealedArtifacts: {
    fixtureVersion: "1.0.0",
    contractVersion: FORGE_VISIONER_SYNTHESIS_CONTRACT_V1.version,
    harnessVersion: FORGE_VISIONER_SYNTHESIS_VERSION,
    probeCount: summarizeVisionerSynthesisContractCoverage(FORGE_VISIONER_SYNTHESIS_CONTRACT_V1).totalProbes,
    visionerSynthesisCategories: VISIONER_SYNTHESIS_CATEGORIES,
    sourceVisionerConstraintBlockGateAtom: "P02-B02-A10",
  },
  prerequisites: [
    "Visioner synthesis contract v1 with measurable synthesis signal, aesthetic and guard probes",
    "Versioned visioner synthesis baseline aligned to contract probe matrix and sealed P02-B02 block gate",
    "Evidence, telemetry and provenance run records",
    "Regression and guard gates integrated with orchestrator verification",
    "Sealed P02-B02 visioner constraint block gate referenced by sourceVisionerConstraintBlockGateAtom",
  ],
  entryCriteria: {
    description:
      "P02-B04-A01 formalizes repo and user context grounding using sealed visioner synthesis artifacts",
    requiresBlockGatePass: true,
    visionerSynthesisRecordRequired: true,
  },
};

export function getForgeP02B03BlockGate(): ForgeBlockGateDefinition {
  return FORGE_P02_B03_BLOCK_GATE_V1;
}

export function getForgeP02B03ToB04Handoff(): VisionerSynthesisBlockHandoffContract {
  return FORGE_P02_B03_TO_B04_HANDOFF_V1;
}

export function validateVisionerSynthesisBlockHandoffContract(
  handoff: VisionerSynthesisBlockHandoffContract,
  evidence: Pick<VisionerSynthesisBlockGateEvidence, "probeCount" | "regressionPassed" | "guardPassed">,
  contract: VisionerSynthesisContract = getActiveVisionerSynthesisContract(),
): { valid: boolean; issues: string[] } {
  const issues: string[] = [];
  const coverage = summarizeVisionerSynthesisContractCoverage(contract);
  ...
  if (handoff.sealedArtifacts.visionerSynthesisCategories.length !== VISIONER_SYNTHESIS_CATEGORIES.length) {
    issues.push("handoff visionerSynthesisCategories incomplete");
  }
  if (handoff.targetBlock.entryAtom !== "P02-B04-A01") {
...
export function buildVisionerSynthesisBlockGateEvidence(
  atomSeals: ForgeBlockAtomSeal[],
  regressionPassed: boolean,
  guardPassed: boolean,
  probeCount: number,
  gitCommit?: string,
  blockId = FORGE_P02_B03_BLOCK_GATE_V1.blockId,
): VisionerSynthesisBlockGateEvidence {
  const handoff = getForgeP02B03ToB04Handoff();
  const handoffValid = validateVisionerSynthesisBlockHandoffContract(handoff, {
    probeCount,
    regressionPassed,
    guardPassed,
  }).valid;
