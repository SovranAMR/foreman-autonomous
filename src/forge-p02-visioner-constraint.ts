...
export const FORGE_VISIONER_CONSTRAINT_VERSION = "1.0.0-a07";
...
// ─── Property and fuzz validation (P02-B02-A07) ─────────────────────────────

export interface VisionerConstraintPropertyViolation {
  propertyId: string;
  detail: string;
}

export interface VisionerConstraintPropertyResult {
  passed: number;
  failed: VisionerConstraintPropertyViolation[];
  total: number;
  allPassed: boolean;
}

export type VisionerConstraintPropertyCheck = {
  id: string;
  description: string;
  check: (contract: VisionerConstraintContract) => string | null;
};

const VISIONER_CONSTRAINT_STRUCTURAL_PROPERTIES: readonly VisionerConstraintPropertyCheck[] = [
  {
    id: "categories_complete",
    description: "All eight visioner constraint categories are declared",
    check: contract => {
      for (const category of VISIONER_CONSTRAINT_CATEGORIES) {
  ...
  },
  {
    ...
    description: "summarizeVisionerConstraintContractCoverage totals match listVisionerConstraintContractProbeIds",
    check: contract => {
      const summary = summarizeVisionerConstraintContractCoverage(contract);
      const ids = listVisionerConstraintContractProbeIds(contract);
      if (summary.totalProbes !== ids.length) {
  ...
  {
    id: "probe_id_prefix",
    description: "Probe ids are namespaced with vcon. prefix",
    check: contract => {
      for (const probe of contract.probes) {
        if (!probe.id.startsWith("vcon.")) {
          return `${probe.id} missing vcon. prefix`;
        }
      }
      return null;
    },
  },
  {
    ...
    check: contract => {
      ...
      const telemetry = probeIds.map((id, index) => {
        const probe = contract.probes.find(p => p.id === id)!;
        return buildVisionerConstraintProbeTelemetry(id, probe.category, index, index);
      });
      const record = buildVisionerConstraintRunRecord(
        buildVisionerConstraintProvenance(
          "property-check",
          {
            ...
            atom: "x",
            purpose: "x",
            sourceBlockGate: buildDefaultSourceBlockGate(),
            probes: [],
          },
          contract,
  ...
  },
  {
    ...
    description: "Synthetic failure/recovery slice record passes validateVisionerConstraintFailureRecoveryRunRecord",
    check: contract => {
      ...
      const telemetry = probeIds.map((id, index) => {
        const probe = contract.probes.find(p => p.id === id)!;
        return buildVisionerConstraintProbeTelemetry(id, probe.category, index, index * 0.5);
      });
      const record = buildVisionerConstraintRunRecord(
        buildVisionerConstraintProvenance(
...
export function runVisionerConstraintPropertyChecks(
  contract: VisionerConstraintContract = getActiveVisionerConstraintContract(),
): VisionerConstraintPropertyResult {
  const failed: VisionerConstraintPropertyViolation[] = [];
  for (const property of VISIONER_CONSTRAINT_STRUCTURAL_PROPERTIES) {
    const detail = property.check(contract);
    if (detail) failed.push({ propertyId: property.id, detail });
  }
  const total = VISIONER_CONSTRAINT_STRUCTURAL_PROPERTIES.length;
  return {
...
export type VisionerConstraintFuzzMutationKind =
  | "flip_expected"
  | "drop_probe"
  | "extra_probe"
  | "rename_probe"
...
export interface VisionerConstraintFuzzMutationCase {
  seed: number;
  kind: VisionerConstraintFuzzMutationKind;
  probeId?: string;
  category?: VisionerConstraintCategory;
}

export interface VisionerConstraintFuzzValidationCaseResult {
  mutation: VisionerConstraintFuzzMutationCase;
  valid: boolean;
  issueKinds: string[];
}

export interface VisionerConstraintFuzzValidationResult {
  seed: number;
  iterations: number;
  rejected: number;
  accepted: number;
  cases: VisionerConstraintFuzzValidationCaseResult[];
  allMutationsRejected: boolean;
}

/** Deterministic PRNG for reproducible fuzz cases (mulberry32). */
export function createVisionerConstraintFuzzRng(seed: number): () => number {
  let state = seed >>> 0;
  return () => {
...
function cloneVisionerConstraintBaseline(fixture: VisionerConstraintBaseline): VisionerConstraintBaseline {
  return {
    ...fixture,
...
function pickVisionerConstraintFuzzTarget(
  fixture: VisionerConstraintBaseline,
  rng: () => number,
): { category: VisionerConstraintCategory; index: number; entry: VisionerConstraintFixtureEntry } {
  const category = VISIONER_CONSTRAINT_CATEGORIES[Math.floor(rng() * VISIONER_CONSTRAINT_CATEGORIES.length)]!;
...
export function applyVisionerConstraintFuzzMutation(
  fixture: VisionerConstraintBaseline,
  mutation: VisionerConstraintFuzzMutationCase,
): VisionerConstraintBaseline {
  const mutated = cloneVisionerConstraintBaseline(fixture);
  const targetCategory = mutation.category ?? VISIONER_CONSTRAINT_CATEGORIES[0]!;
  ...
  switch (mutation.kind) {
    ...
    case "extra_probe":
      mutated.probes = [
        ...mutated.probes,
        {
          id: `vcon.fuzz.extra.${mutation.seed}`,
    ...
    case "flip_category": {
      const probeId = mutation.probeId ?? categoryEntries[0]!.id;
      const entry = mutated.probes.find(e => e.id === probeId) ?? categoryEntries[0]!;
      const other = VISIONER_CONSTRAINT_CATEGORIES.find(c => c !== entry.category)!;
...
export function generateVisionerConstraintFuzzMutationCases(
  fixture: VisionerConstraintBaseline,
  seed: number,
  iterations: number,
): VisionerConstraintFuzzMutationCase[] {
  const rng = createVisionerConstraintFuzzRng(seed);
  const kinds: VisionerConstraintFuzzMutationKind[] = [
    "flip_expected",
    "drop_probe",
    "extra_probe",
    "rename_probe",
  ...
  const cases: VisionerConstraintFuzzMutationCase[] = [];

  for (let i = 0; i < iterations; i++) {
    const kind = kinds[Math.floor(rng() * kinds.length)]!;
    const target = pickVisionerConstraintFuzzTarget(fixture, rng);
    cases.push({
      seed: seed + i,
      kind,
...
/** Fuzz harness: mutated fixtures must fail contract validation (P02-B02-A07). */
export function runVisionerConstraintFuzzValidation(
  fixture: VisionerConstraintBaseline,
  contract: VisionerConstraintContract = getActiveVisionerConstraintContract(),
  seed = 42,
  iterations = 24,
): VisionerConstraintFuzzValidationResult {
  const cases = generateVisionerConstraintFuzzMutationCases(fixture, seed, iterations);
  const results: VisionerConstraintFuzzValidationCaseResult[] = [];
  let rejected = 0;
  let accepted = 0;

  for (const mutation of cases) {
    const mutated = applyVisionerConstraintFuzzMutation(fixture, mutation);
    const validation = validateVisionerConstraintAgainstContract(mutated, contract);
...
export type VisionerConstraintRunRecordFuzzKind =
  | "drop_evidence"
  | "drop_telemetry"
  | "wrong_total"
  | "wrong_slice_atom"
...
export interface VisionerConstraintRunRecordFuzzCase {
  kind: VisionerConstraintRunRecordFuzzKind;
  probeId?: string;
}

export function applyVisionerConstraintRunRecordFuzzMutation(
  record: VisionerConstraintRunRecord,
  mutation: VisionerConstraintRunRecordFuzzCase,
): VisionerConstraintRunRecord {
  const cloned: VisionerConstraintRunRecord = {
  ...
  switch (mutation.kind) {
    ...
    case "wrong_slice_atom":
      cloned.provenance = { ...cloned.provenance, sliceAtom: "P02-B02-A99" };
      break;
    case "wrong_slice_categories":
      cloned.provenance = {
        ...cloned.provenance,
        sliceCategories: ["constraint_versioning"],
      };
      break;
  }

  cloned.summary = buildVisionerConstraintRunRecord(
    cloned.provenance,
    cloned.evidence,
    cloned.telemetry,
  ).summary;
...
function resolveVisionerConstraintRunRecordValidator(
  record: VisionerConstraintRunRecord,
): (
  record: VisionerConstraintRunRecord,
  contract: VisionerConstraintContract,
) => VisionerConstraintRunValidationResult {
  return record.provenance.sliceAtom === "P02-B02-A06"
    ? validateVisionerConstraintFailureRecoveryRunRecord
    : validateVisionerConstraintRunRecord;
}

/** Fuzz harness: tampered run records must fail validation deterministically (P02-B02-A07). */
export function runVisionerConstraintRunRecordFuzzValidation(
  record: VisionerConstraintRunRecord,
  contract: VisionerConstraintContract = getActiveVisionerConstraintContract(),
): { validBaseline: boolean; mutationsRejected: number; mutationsAccepted: number } {
  const validate = resolveVisionerConstraintRunRecordValidator(record);
  const baseline = validate(record, contract);
  const probeId = record.evidence[0]?.probeId;
  const mutations: VisionerConstraintRunRecordFuzzCase[] = [
  ...
  for (const mutation of mutations) {
    const mutated = applyVisionerConstraintRunRecordFuzzMutation(record, mutation);
...
// ─── Probe regression detection (P02-B02-A08) ────────────────────────────────

export interface VisionerConstraintProbeRegressionReport {
...
/**
 * Compare visioner constraint run records and detect probe alignment regressions.
 * A regression = probe aligned in prior run but misaligned in current run.
 */
export function detectVisionerConstraintProbeRegression(
  prior: VisionerConstraintRunRecord,
  current: VisionerConstraintRunRecord,
): VisionerConstraintProbeRegressionReport {
...
// ─── Guard controls (P02-B02-A09 foundation, used by A08 regression gate) ───

export interface ForgeVisionerConstraintGuardControls {
  atom: string;
  adversarial: {
...
export interface VisionerConstraintGuardCheckIssue {
  domain: "adversarial" | "performance" | "cost" | "safety";
  code: string;
  detail: string;
}

export interface VisionerConstraintGuardCheckResult {
  passed: boolean;
  issues: VisionerConstraintGuardCheckIssue[];
  metrics: {
...
export interface VisionerConstraintAdversarialGuardScenario {
  id: string;
  description: string;
  build: (record: VisionerConstraintRunRecord) => VisionerConstraintRunRecord;
  expectRejected: true;
}

export const FORGE_VISIONER_CONSTRAINT_GUARD_CONTROLS_V1: ForgeVisionerConstraintGuardControls = {
  atom: "P02-B02-A09",
  adversarial: {
    rejectTamperedRecords: true,
    rejectFalseAlignment: true,
...
export function getForgeVisionerConstraintGuardControls(): ForgeVisionerConstraintGuardControls {
  return FORGE_VISIONER_CONSTRAINT_GUARD_CONTROLS_V1;
}

function parseVisionerConstraintIsoDurationMs(startedAt: string, completedAt: string): number {
...
export function summarizeVisionerConstraintTelemetry(telemetry: VisionerConstraintProbeTelemetry[]): {
  suiteDurationMs: number;
  maxProbeDurationMs: number;
} {
...
export function detectVisionerConstraintEvidenceSummaryMismatch(
  record: VisionerConstraintRunRecord,
): string | null {
  let alignedCount = 0;
  for (const item of record.evidence) {
...
export function detectVisionerConstraintFalseAlignment(record: VisionerConstraintRunRecord): string[] {
...
export function validateVisionerConstraintSafety(
  record: VisionerConstraintRunRecord,
  controls: ForgeVisionerConstraintGuardControls = getForgeVisionerConstraintGuardControls(),
): VisionerConstraintGuardCheckIssue[] {
  const issues: VisionerConstraintGuardCheckIssue[] = [];
  for (const item of record.evidence) {
...
export function validateVisionerConstraintPerformance(
  record: VisionerConstraintRunRecord,
  controls: ForgeVisionerConstraintGuardControls = getForgeVisionerConstraintGuardControls(),
): VisionerConstraintGuardCheckIssue[] {
  const issues: VisionerConstraintGuardCheckIssue[] = [];
  const { suiteDurationMs, maxProbeDurationMs } = summarizeVisionerConstraintTelemetry(record.telemetry);
  const wallClockMs = parseVisionerConstraintIsoDurationMs(
...
export function validateVisionerConstraintCost(
  totalCostUsd: number,
  llmCalls: number,
  controls: ForgeVisionerConstraintGuardControls = getForgeVisionerConstraintGuardControls(),
): VisionerConstraintGuardCheckIssue[] {
...
export function buildVisionerConstraintAdversarialGuardScenarios(): VisionerConstraintAdversarialGuardScenario[] {
  return [
    {
...
export function runVisionerConstraintAdversarialGuardChecks(
  fixtureRecord: VisionerConstraintRunRecord,
  contract: VisionerConstraintContract = getActiveVisionerConstraintContract(),
  ...
  for (const scenario of scenarios) {
    ...
    const validation = validateVisionerConstraintRunRecord(tampered, contract);
    const falseAlignment = detectVisionerConstraintFalseAlignment(tampered);
    const summaryMismatch = detectVisionerConstraintEvidenceSummaryMismatch(tampered);
...
export function validateForgeVisionerConstraintGuard(
  record: VisionerConstraintRunRecord,
  options: {
    totalCostUsd?: number;
    llmCalls?: number;
    contract?: VisionerConstraintContract;
    controls?: ForgeVisionerConstraintGuardControls;
  } = {},
): VisionerConstraintGuardCheckResult {
  const controls = options.controls ?? getForgeVisionerConstraintGuardControls();
  const contract = options.contract ?? getActiveVisionerConstraintContract();
  const totalCostUsd = options.totalCostUsd ?? 0;
  const llmCalls = options.llmCalls ?? 0;
  const issues: VisionerConstraintGuardCheckIssue[] = [];

  issues.push(...validateVisionerConstraintPerformance(record, controls));
  issues.push(...validateVisionerConstraintCost(totalCostUsd, llmCalls, controls));
  issues.push(...validateVisionerConstraintSafety(record, controls));

  const falseAlignment = detectVisionerConstraintFalseAlignment(record);
  ...
  const summaryMismatch = detectVisionerConstraintEvidenceSummaryMismatch(record);
  if (summaryMismatch) {
    issues.push({
      domain: "adversarial",
  ...
  const telemetrySummary = summarizeVisionerConstraintTelemetry(record.telemetry);
  const wallClockMs = parseVisionerConstraintIsoDurationMs(
    record.provenance.startedAt,
    record.provenance.completedAt,
  );
