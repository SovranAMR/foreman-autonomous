...
// ─── Probe regression detection (P02-B09-A08) ────────────────────────────────

export interface VisionerApprovalProbeRegressionReport {
...
/**
 * Compare visioner approval run records and detect probe alignment regressions.
 * A regression = probe aligned in prior run but misaligned in current run.
 */
export function detectVisionerApprovalProbeRegression(
  prior: VisionerApprovalRunRecord,
  current: VisionerApprovalRunRecord,
): VisionerApprovalProbeRegressionReport {
...
// ─── Guard controls (P02-B09-A09 foundation, used by A08 regression gate) ───

export interface ForgeVisionerApprovalGuardControls {
  atom: string;
  adversarial: {
...
export interface VisionerApprovalGuardCheckIssue {
  domain: "adversarial" | "performance" | "cost" | "safety";
  code: string;
  detail: string;
}

export interface VisionerApprovalGuardCheckResult {
  passed: boolean;
  issues: VisionerApprovalGuardCheckIssue[];
  metrics: {
...
export interface VisionerApprovalAdversarialGuardScenario {
  id: string;
  description: string;
  build: (record: VisionerApprovalRunRecord) => VisionerApprovalRunRecord;
  expectRejected: true;
}

export const FORGE_VISIONER_APPROVAL_GUARD_CONTROLS_V1: ForgeVisionerApprovalGuardControls = {
  atom: "P02-B09-A09",
  adversarial: {
    rejectTamperedRecords: true,
    rejectFalseAlignment: true,
...
export function getForgeVisionerApprovalGuardControls(): ForgeVisionerApprovalGuardControls {
  return FORGE_VISIONER_APPROVAL_GUARD_CONTROLS_V1;
}

function parseVisionerApprovalIsoDurationMs(startedAt: string, completedAt: string): number {
...
export function summarizeVisionerApprovalTelemetry(telemetry: VisionerApprovalProbeTelemetry[]): {
  suiteDurationMs: number;
  maxProbeDurationMs: number;
} {
...
export function detectVisionerApprovalEvidenceSummaryMismatch(
  record: VisionerApprovalRunRecord,
): string | null {
  let alignedCount = 0;
  for (const item of record.evidence) {
...
export function detectVisionerApprovalFalseAlignment(record: VisionerApprovalRunRecord): string[] {
...
export function validateVisionerApprovalSafety(
  record: VisionerApprovalRunRecord,
  controls: ForgeVisionerApprovalGuardControls = getForgeVisionerApprovalGuardControls(),
): VisionerApprovalGuardCheckIssue[] {
  const issues: VisionerApprovalGuardCheckIssue[] = [];
  for (const item of record.evidence) {
...
export function validateVisionerApprovalPerformance(
  record: VisionerApprovalRunRecord,
  controls: ForgeVisionerApprovalGuardControls = getForgeVisionerApprovalGuardControls(),
): VisionerApprovalGuardCheckIssue[] {
  const issues: VisionerApprovalGuardCheckIssue[] = [];
  const { suiteDurationMs, maxProbeDurationMs } = summarizeVisionerApprovalTelemetry(record.telemetry);
  const wallClockMs = parseVisionerApprovalIsoDurationMs(
...
export function validateVisionerApprovalCost(
  totalCostUsd: number,
  llmCalls: number,
  controls: ForgeVisionerApprovalGuardControls = getForgeVisionerApprovalGuardControls(),
): VisionerApprovalGuardCheckIssue[] {
...
export function buildVisionerApprovalAdversarialGuardScenarios(): VisionerApprovalAdversarialGuardScenario[] {
  return [
    {
...
export function runVisionerApprovalAdversarialGuardChecks(
  fixtureRecord: VisionerApprovalRunRecord,
  contract: VisionerApprovalContract = getActiveVisionerApprovalContract(),
  ...
  for (const scenario of scenarios) {
    ...
    const validate = resolveVisionerApprovalRunRecordValidator(tampered);
    const validation = validate(tampered, contract);
    const falseAlignment = detectVisionerApprovalFalseAlignment(tampered);
    const summaryMismatch = detectVisionerApprovalEvidenceSummaryMismatch(tampered);
...
export function validateForgeVisionerApprovalGuard(
  record: VisionerApprovalRunRecord,
  options: {
    totalCostUsd?: number;
    llmCalls?: number;
    contract?: VisionerApprovalContract;
    controls?: ForgeVisionerApprovalGuardControls;
  } = {},
): VisionerApprovalGuardCheckResult {
  const controls = options.controls ?? getForgeVisionerApprovalGuardControls();
  const contract = options.contract ?? getActiveVisionerApprovalContract();
  const totalCostUsd = options.totalCostUsd ?? 0;
  const llmCalls = options.llmCalls ?? 0;
  const issues: VisionerApprovalGuardCheckIssue[] = [];

  issues.push(...validateVisionerApprovalPerformance(record, controls));
  issues.push(...validateVisionerApprovalCost(totalCostUsd, llmCalls, controls));
  issues.push(...validateVisionerApprovalSafety(record, controls));

  const falseAlignment = detectVisionerApprovalFalseAlignment(record);
  ...
  const summaryMismatch = detectVisionerApprovalEvidenceSummaryMismatch(record);
  if (summaryMismatch) {
    issues.push({
      domain: "adversarial",
  ...
  const telemetrySummary = summarizeVisionerApprovalTelemetry(record.telemetry);
  const wallClockMs = parseVisionerApprovalIsoDurationMs(
    record.provenance.startedAt,
    record.provenance.completedAt,
  );
...
export interface VisionerApprovalBlockGateEvidence {
  blockId: string;
  atom: string;
  sealedAt: string;
...
export interface VisionerApprovalBlockHandoffContract {
  version: string;
  atom: string;
  sourceBlock: {
    blockId: string;
  ...
  sealedArtifacts: {
    ...
    harnessVersion: string;
    probeCount: number;
    visionerApprovalCategories: readonly VisionerApprovalCategory[];
    sourceVisionerScoringBlockGateAtom: string;
  };
  prerequisites: readonly string[];
  entryCriteria: {
    description: string;
    requiresBlockGatePass: true;
    visionerApprovalRecordRequired: true;
  };
}

export const FORGE_P02_B09_BLOCK_GATE_V1: ForgeBlockGateDefinition = {
  version: "1.0.0",
  atom: "P02-B09-A10",
  blockId: "P02-B09",
  title: "Kullanıcı approval ve steering",
  requiredAtomIds: [
    "P02-B09-A01",
    "P02-B09-A02",
    "P02-B09-A03",
  ...
  checks: [
    { id: "fixture_contract_alignment", atomId: "P02-B09-A01", description: "Visioner approval baseline aligns with typed contract and P02-B08 block gate handoff" },
    { id: "typed_contract_coverage", atomId: "P02-B09-A02", description: "Contract declares measurable probes for all visioner approval categories" },
    { id: "probe_matrix_aligned", atomId: "P02-B09-A03", description: "Visioner approval probe matrix executes with zero unexpected mismatches" },
    { id: "boundary_disposition_coverage", atomId: "P02-B09-A04", description: "Contract covers observed, gap, failure, recovery and NO-GO dispositions" },
    { id: "failure_recovery_nogo", atomId: "P02-B09-A05", description: "Failure, recovery and NO-GO probes are declared and exercised" },
    { id: "evidence_telemetry_provenance", atomId: "P02-B09-A06", description: "Run record carries evidence, telemetry and provenance" },
    { id: "property_and_fuzz", atomId: "P02-B09-A07", description: "Structural property and fuzz validation reject tampered inputs" },
    { id: "regression_gate", atomId: "P02-B09-A08", description: "Regression gate passes on canonical visioner approval matrix" },
    { id: "guard_controls", atomId: "P02-B09-A09", description: "Adversarial, performance, cost and safety guard controls pass" },
    { id: "block_gate_sealed", atomId: "P02-B09-A10", description: "Block gate evidence sealed with valid B10 handoff contract" },
  ] satisfies readonly ForgeBlockGateCheck[],
};

export const FORGE_P02_B09_TO_B10_HANDOFF_V1: VisionerApprovalBlockHandoffContract = {
  version: "1.0.0",
  atom: "P02-B09-A10",
  sourceBlock: {
    blockId: "P02-B09",
    title: "Kullanıcı approval ve steering",
    completedAtoms: FORGE_P02_B09_BLOCK_GATE_V1.requiredAtomIds,
  },
  targetBlock: {
    blockId: "P02-B10",
    title: "Vizyoner phase gate",
    entryAtom: "P02-B10-A01",
  },
  sealedArtifacts: {
    fixtureVersion: "1.0.0",
    contractVersion: FORGE_VISIONER_APPROVAL_CONTRACT_V1.version,
    harnessVersion: FORGE_VISIONER_APPROVAL_VERSION,
    probeCount: summarizeVisionerApprovalContractCoverage(FORGE_VISIONER_APPROVAL_CONTRACT_V1).totalProbes,
    visionerApprovalCategories: VISIONER_APPROVAL_CATEGORIES,
    sourceVisionerScoringBlockGateAtom: "P02-B08-A10",
  },
  prerequisites: [
    "Visioner approval contract v1 with measurable approval signal, steering and guard probes",
    "Versioned visioner approval baseline aligned to contract probe matrix and sealed P02-B08 block gate",
    "Evidence, telemetry and provenance run records",
    "Regression and guard gates integrated with orchestrator verification",
    "Sealed P02-B08 visioner scoring block gate referenced by sourceVisionerScoringBlockGateAtom",
  ],
  entryCriteria: {
    description:
      "P02-B10-A01 formalizes visioner phase gate using sealed user approval and steering artifacts",
    requiresBlockGatePass: true,
    visionerApprovalRecordRequired: true,
  },
};

export function getForgeP02B09BlockGate(): ForgeBlockGateDefinition {
  return FORGE_P02_B09_BLOCK_GATE_V1;
}

export function getForgeP02B09ToB10Handoff(): VisionerApprovalBlockHandoffContract {
  return FORGE_P02_B09_TO_B10_HANDOFF_V1;
}

export function validateVisionerApprovalBlockHandoffContract(
  handoff: VisionerApprovalBlockHandoffContract,
  evidence: Pick<VisionerApprovalBlockGateEvidence, "probeCount" | "regressionPassed" | "guardPassed">,
  contract: VisionerApprovalContract = getActiveVisionerApprovalContract(),
): { valid: boolean; issues: string[] } {
  const issues: string[] = [];
  const coverage = summarizeVisionerApprovalContractCoverage(contract);
  ...
  if (handoff.sealedArtifacts.visionerApprovalCategories.length !== VISIONER_APPROVAL_CATEGORIES.length) {
    issues.push("handoff visionerApprovalCategories incomplete");
  }
  if (handoff.targetBlock.entryAtom !== "P02-B10-A01") {
...
export function buildVisionerApprovalBlockGateEvidence(
  atomSeals: ForgeBlockAtomSeal[],
  regressionPassed: boolean,
  guardPassed: boolean,
  probeCount: number,
  gitCommit?: string,
  blockId = FORGE_P02_B09_BLOCK_GATE_V1.blockId,
): VisionerApprovalBlockGateEvidence {
  const handoff = getForgeP02B09ToB10Handoff();
  const handoffValid = validateVisionerApprovalBlockHandoffContract(handoff, {
    probeCount,
    regressionPassed,
    guardPassed,
  }).valid;
