...
export const FORGE_VISIONER_UNCERTAINTY_VERSION = "1.0.0-a10";
...
// ─── Guard controls (P02-B06-A09 foundation, used by A08 regression gate) ───

export interface ForgeVisionerUncertaintyGuardControls {
  atom: string;
  adversarial: {
...
export interface VisionerUncertaintyGuardCheckIssue {
  domain: "adversarial" | "performance" | "cost" | "safety";
  code: string;
  detail: string;
}

export interface VisionerUncertaintyGuardCheckResult {
  passed: boolean;
  issues: VisionerUncertaintyGuardCheckIssue[];
  metrics: {
...
export interface VisionerUncertaintyAdversarialGuardScenario {
  id: string;
  description: string;
  build: (record: VisionerUncertaintyRunRecord) => VisionerUncertaintyRunRecord;
  expectRejected: true;
}

export const FORGE_VISIONER_UNCERTAINTY_GUARD_CONTROLS_V1: ForgeVisionerUncertaintyGuardControls = {
  atom: "P02-B06-A09",
  adversarial: {
    rejectTamperedRecords: true,
    rejectFalseAlignment: true,
...
export function getForgeVisionerUncertaintyGuardControls(): ForgeVisionerUncertaintyGuardControls {
  return FORGE_VISIONER_UNCERTAINTY_GUARD_CONTROLS_V1;
}

function parseVisionerUncertaintyIsoDurationMs(startedAt: string, completedAt: string): number {
...
export function summarizeVisionerUncertaintyTelemetry(telemetry: VisionerUncertaintyProbeTelemetry[]): {
  suiteDurationMs: number;
  maxProbeDurationMs: number;
} {
...
export function detectVisionerUncertaintyEvidenceSummaryMismatch(
  record: VisionerUncertaintyRunRecord,
): string | null {
  let alignedCount = 0;
  for (const item of record.evidence) {
...
export function detectVisionerUncertaintyFalseAlignment(record: VisionerUncertaintyRunRecord): string[] {
...
export function validateVisionerUncertaintySafety(
  record: VisionerUncertaintyRunRecord,
  controls: ForgeVisionerUncertaintyGuardControls = getForgeVisionerUncertaintyGuardControls(),
): VisionerUncertaintyGuardCheckIssue[] {
  const issues: VisionerUncertaintyGuardCheckIssue[] = [];
  for (const item of record.evidence) {
...
export function validateVisionerUncertaintyPerformance(
  record: VisionerUncertaintyRunRecord,
  controls: ForgeVisionerUncertaintyGuardControls = getForgeVisionerUncertaintyGuardControls(),
): VisionerUncertaintyGuardCheckIssue[] {
  const issues: VisionerUncertaintyGuardCheckIssue[] = [];
  const { suiteDurationMs, maxProbeDurationMs } = summarizeVisionerUncertaintyTelemetry(record.telemetry);
  const wallClockMs = parseVisionerUncertaintyIsoDurationMs(
...
export function validateVisionerUncertaintyCost(
  totalCostUsd: number,
  llmCalls: number,
  controls: ForgeVisionerUncertaintyGuardControls = getForgeVisionerUncertaintyGuardControls(),
): VisionerUncertaintyGuardCheckIssue[] {
...
export function buildVisionerUncertaintyAdversarialGuardScenarios(): VisionerUncertaintyAdversarialGuardScenario[] {
  return [
    {
...
export function runVisionerUncertaintyAdversarialGuardChecks(
  fixtureRecord: VisionerUncertaintyRunRecord,
  contract: VisionerUncertaintyContract = getActiveVisionerUncertaintyContract(),
  ...
  for (const scenario of scenarios) {
    ...
    const validation = validateVisionerUncertaintyRunRecord(tampered, contract);
    const falseAlignment = detectVisionerUncertaintyFalseAlignment(tampered);
    const summaryMismatch = detectVisionerUncertaintyEvidenceSummaryMismatch(tampered);
...
export function validateForgeVisionerUncertaintyGuard(
  record: VisionerUncertaintyRunRecord,
  options: {
    totalCostUsd?: number;
    llmCalls?: number;
    contract?: VisionerUncertaintyContract;
    controls?: ForgeVisionerUncertaintyGuardControls;
  } = {},
): VisionerUncertaintyGuardCheckResult {
  const controls = options.controls ?? getForgeVisionerUncertaintyGuardControls();
  const contract = options.contract ?? getActiveVisionerUncertaintyContract();
  const totalCostUsd = options.totalCostUsd ?? 0;
  const llmCalls = options.llmCalls ?? 0;
  const issues: VisionerUncertaintyGuardCheckIssue[] = [];

  issues.push(...validateVisionerUncertaintyPerformance(record, controls));
  issues.push(...validateVisionerUncertaintyCost(totalCostUsd, llmCalls, controls));
  issues.push(...validateVisionerUncertaintySafety(record, controls));

  const falseAlignment = detectVisionerUncertaintyFalseAlignment(record);
  ...
  const summaryMismatch = detectVisionerUncertaintyEvidenceSummaryMismatch(record);
  if (summaryMismatch) {
    issues.push({
      domain: "adversarial",
  ...
  const telemetrySummary = summarizeVisionerUncertaintyTelemetry(record.telemetry);
  const wallClockMs = parseVisionerUncertaintyIsoDurationMs(
    record.provenance.startedAt,
    record.provenance.completedAt,
  );
...
export interface VisionerUncertaintyBlockGateEvidence {
  blockId: string;
  atom: string;
  sealedAt: string;
...
export interface VisionerUncertaintyBlockHandoffContract {
  version: string;
  atom: string;
  sourceBlock: {
    blockId: string;
  ...
  sealedArtifacts: {
    ...
    harnessVersion: string;
    probeCount: number;
    visionerUncertaintyCategories: readonly VisionerUncertaintyCategory[];
    sourceVisionerResearchTriggerBlockGateAtom: string;
  };
  prerequisites: readonly string[];
  entryCriteria: {
    description: string;
    requiresBlockGatePass: true;
    visionerUncertaintyRecordRequired: true;
  };
}

export const FORGE_P02_B06_BLOCK_GATE_V1: ForgeBlockGateDefinition = {
  version: "1.0.0",
  atom: "P02-B06-A10",
  blockId: "P02-B06",
  ...
  checks: [
    { id: "fixture_contract_alignment", atomId: "P02-B06-A01", description: "Visioner uncertainty baseline aligns with typed contract and P02-B05 block gate handoff" },
    { id: "typed_contract_coverage", atomId: "P02-B06-A02", description: "Contract declares measurable probes for all visioner uncertainty categories" },
    { id: "probe_matrix_aligned", atomId: "P02-B06-A03", description: "Visioner uncertainty probe matrix executes with zero unexpected mismatches" },
    { id: "boundary_disposition_coverage", atomId: "P02-B06-A04", description: "Contract covers observed, gap, failure, recovery and NO-GO dispositions" },
    { id: "failure_recovery_nogo", atomId: "P02-B06-A05", description: "Failure, recovery and NO-GO probes are declared and exercised" },
    { id: "evidence_telemetry_provenance", atomId: "P02-B06-A06", description: "Run record carries evidence, telemetry and provenance" },
    { id: "property_and_fuzz", atomId: "P02-B06-A07", description: "Structural property and fuzz validation reject tampered inputs" },
    { id: "regression_gate", atomId: "P02-B06-A08", description: "Regression gate passes on canonical visioner uncertainty matrix" },
    { id: "guard_controls", atomId: "P02-B06-A09", description: "Adversarial, performance, cost and safety guard controls pass" },
    { id: "block_gate_sealed", atomId: "P02-B06-A10", description: "Block gate evidence sealed with valid B07 handoff contract" },
  ] satisfies readonly ForgeBlockGateCheck[],
};

export const FORGE_P02_B06_TO_B07_HANDOFF_V1: VisionerUncertaintyBlockHandoffContract = {
  version: "1.0.0",
  atom: "P02-B06-A10",
  sourceBlock: {
    blockId: "P02-B06",
    title: "Uncertainty ve clarification policy",
    completedAtoms: FORGE_P02_B06_BLOCK_GATE_V1.requiredAtomIds,
  },
  targetBlock: {
    blockId: "P02-B07",
    title: "Alternatif vizyon üretimi",
    entryAtom: "P02-B07-A01",
  },
  sealedArtifacts: {
    fixtureVersion: "1.0.0",
    contractVersion: FORGE_VISIONER_UNCERTAINTY_CONTRACT_V1.version,
    harnessVersion: FORGE_VISIONER_UNCERTAINTY_VERSION,
    probeCount: summarizeVisionerUncertaintyContractCoverage(FORGE_VISIONER_UNCERTAINTY_CONTRACT_V1).totalProbes,
    visionerUncertaintyCategories: VISIONER_UNCERTAINTY_CATEGORIES,
    sourceVisionerResearchTriggerBlockGateAtom: "P02-B05-A10",
  },
  prerequisites: [
    "Visioner uncertainty contract v1 with measurable confidence, clarification and guard probes",
    "Versioned visioner uncertainty baseline aligned to contract probe matrix and sealed P02-B05 block gate",
    "Evidence, telemetry and provenance run records",
    "Regression and guard gates integrated with orchestrator verification",
    "Sealed P02-B05 visioner research trigger block gate referenced by sourceVisionerResearchTriggerBlockGateAtom",
  ],
  entryCriteria: {
    description:
      "P02-B07-A01 formalizes alternative vision generation using sealed visioner uncertainty artifacts",
    requiresBlockGatePass: true,
    visionerUncertaintyRecordRequired: true,
  },
};

export function getForgeP02B06BlockGate(): ForgeBlockGateDefinition {
  return FORGE_P02_B06_BLOCK_GATE_V1;
}

export function getForgeP02B06ToB07Handoff(): VisionerUncertaintyBlockHandoffContract {
  return FORGE_P02_B06_TO_B07_HANDOFF_V1;
}

export function validateVisionerUncertaintyBlockHandoffContract(
  handoff: VisionerUncertaintyBlockHandoffContract,
  evidence: Pick<VisionerUncertaintyBlockGateEvidence, "probeCount" | "regressionPassed" | "guardPassed">,
  contract: VisionerUncertaintyContract = getActiveVisionerUncertaintyContract(),
  ...
  if (handoff.sealedArtifacts.visionerUncertaintyCategories.length !== VISIONER_UNCERTAINTY_CATEGORIES.length) {
    issues.push("handoff visionerUncertaintyCategories incomplete");
  }
  if (handoff.targetBlock.entryAtom !== "P02-B07-A01") {
...
export function buildVisionerUncertaintyBlockGateEvidence(
  atomSeals: ForgeBlockAtomSeal[],
  regressionPassed: boolean,
  guardPassed: boolean,
  probeCount: number,
  gitCommit?: string,
  blockId = FORGE_P02_B06_BLOCK_GATE_V1.blockId,
): VisionerUncertaintyBlockGateEvidence {
  const handoff = getForgeP02B06ToB07Handoff();
  const handoffValid = validateVisionerUncertaintyBlockHandoffContract(handoff, {
    probeCount,
    regressionPassed,
    guardPassed,
  }).valid;
