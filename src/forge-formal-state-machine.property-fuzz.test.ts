import { describe, it } from "node:test";
import assert from "node:assert/strict";
import { loadFormalStateMachineFixture } from "./forge-formal-state-machine-harness.js";
import {
  FORGE_FORMAL_STATE_MACHINE_CONTRACT_V1,
  buildFormalStateMachineProbeEvidence,
  buildFormalStateMachineProbeTelemetry,
  buildFormalStateMachineProvenance,
  buildFormalStateMachineRunRecord,
  createFormalStateMachineFuzzRng,
  getActiveFormalStateMachineContract,
  listFormalStateMachineContractProbeIds,
  runFormalStateMachineFuzzValidation,
  runFormalStateMachinePropertyChecks,
  runFormalStateMachineRunRecordFuzzValidation,
  validateFormalStateMachineRunRecord,
} from "./forge-formal-state-machine.js";

describe("Forge Formal State Machine — P01-B03-A07 property checks", () => {
  it("passes all structural properties on canonical contract", () => {
    const result = runFormalStateMachinePropertyChecks(FORGE_FORMAL_STATE_MACHINE_CONTRACT_V1);
    assert.equal(result.allPassed, true, result.failed.map(f => `${f.propertyId}: ${f.detail}`).join("\n"));
    assert.equal(result.passed, result.total);
    assert.ok(result.total >= 7);
  });

  it("createFormalStateMachineFuzzRng is deterministic for reproducible fuzz seeds", () => {
    const rngA = createFormalStateMachineFuzzRng(1337);
    const rngB = createFormalStateMachineFuzzRng(1337);
    const seqA = Array.from({ length: 5 }, () => rngA());
    const seqB = Array.from({ length: 5 }, () => rngB());
    assert.deepEqual(seqA, seqB);
    assert.notDeepEqual(seqA, Array.from({ length: 5 }, () => createFormalStateMachineFuzzRng(1338)()));
  });
});

describe("Forge Formal State Machine — P01-B03-A07 fixture fuzz validation", () => {
  it("rejects all deterministic fixture mutations", () => {
    const fixture = loadFormalStateMachineFixture();
    const contract = getActiveFormalStateMachineContract();

    for (const seed of [42, 99, 20260718]) {
      const fuzz = runFormalStateMachineFuzzValidation(fixture, contract, seed, 24);
      assert.equal(fuzz.iterations, 24);
      assert.equal(fuzz.rejected, 24, `seed=${seed} accepted=${fuzz.accepted}`);
      assert.equal(fuzz.allMutationsRejected, true);
      for (const item of fuzz.cases) {
        assert.equal(item.valid, false, `${item.mutation.kind}@${item.mutation.probeId} should fail`);
        assert.ok(item.issueKinds.length > 0);
      }
    }
  });
});

describe("Forge Formal State Machine — P01-B03-A07 run record fuzz validation", () => {
  it("accepts valid formal state machine record and rejects corrupted mutations", () => {
    const fixture = loadFormalStateMachineFixture();
    const contract = getActiveFormalStateMachineContract();
    const probeIds = listFormalStateMachineContractProbeIds(contract);
    const startedAt = "2026-07-18T22:00:00.000Z";
    const completedAt = "2026-07-18T22:00:01.000Z";

    const evidence = probeIds.map(id => {
      const probe = contract.probes.find(p => p.id === id)!;
      return buildFormalStateMachineProbeEvidence(
        id,
        probe.category,
        probe.expected,
        probe.expected,
        true,
        probe.criterion,
        "synthetic",
        probe.disposition,
        startedAt,
      );
    });

    const telemetry = probeIds.map((id, index) => {
      const probe = contract.probes.find(p => p.id === id)!;
      return buildFormalStateMachineProbeTelemetry(id, probe.category, index, index * 0.05);
    });

    const provenance = buildFormalStateMachineProvenance(
      "property-fuzz-run",
      fixture,
      contract,
      startedAt,
      completedAt,
      probeIds.length,
    );
    const record = buildFormalStateMachineRunRecord(provenance, evidence, telemetry);

    assert.equal(validateFormalStateMachineRunRecord(record, contract).valid, true);

    const fuzz = runFormalStateMachineRunRecordFuzzValidation(record, contract);
    assert.equal(fuzz.validBaseline, true);
    assert.equal(fuzz.mutationsAccepted, 0);
    assert.equal(fuzz.mutationsRejected, 3);
  });
});
