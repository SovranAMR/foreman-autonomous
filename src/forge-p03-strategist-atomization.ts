...
// ─── Probe regression detection (P03-B03-A08) ────────────────────────────────

export interface StrategistAtomizationProbeRegressionReport {
...
/**
 * Compare atomization run records and detect probe alignment regressions.
 * A regression = probe aligned in prior run but misaligned in current run.
 */
export function detectStrategistAtomizationProbeRegression(
  prior: StrategistAtomizationRunRecord,
  current: StrategistAtomizationRunRecord,
): StrategistAtomizationProbeRegressionReport {
...
export interface StrategistAtomizationForgeRegressionResult {
  atom: "P03-B03-A08";
  passed: boolean;
  productionSlice: StrategistAtomizationProductionSliceResult;
  propertyFuzzSlice: StrategistAtomizationPropertyFuzzSliceResult;
  record: StrategistAtomizationRunRecord;
  recordValid: boolean;
  priorRecordValid: boolean;
  validationIssues: string[];
  priorValidationIssues: string[];
  probeRegression: StrategistAtomizationProbeRegressionReport | null;
  detail: string;
}

/**
 * Execute atomization probes, validate production slice + run record, property/fuzz gates,
 * and optionally detect regression vs prior run. Forge pipeline integration gate (P03-B03-A08).
 */
export function runStrategistAtomizationForgeRegression(
  priorRecord?: StrategistAtomizationRunRecord,
): StrategistAtomizationForgeRegressionResult {
  const fixture = loadStrategistAtomizationBaseline();
  const contract = getActiveStrategistAtomizationContract();
  const productionSlice = runStrategistAtomizationProductionSlice(fixture);
  const propertyFuzzSlice = runStrategistAtomizationPropertyFuzzSlice(fixture);
  const record = runStrategistAtomizationProbesWithRecord(fixture);
  ...
  if (priorRecord) {
    const priorValidation = validateStrategistAtomizationRunRecord(priorRecord, contract);
  ...
  const probeRegression = priorRecord
    ? detectStrategistAtomizationProbeRegression(priorRecord, record)
    : null;
...
// ─── Guard controls (P03-B03-A09) ─────────────────────────────────────────────

export interface ForgeStrategistAtomizationGuardControls {
  atom: string;
  adversarial: {
...
export interface StrategistAtomizationGuardCheckIssue {
  domain: "adversarial" | "performance" | "cost" | "safety";
  code: string;
  detail: string;
}

export interface StrategistAtomizationGuardCheckResult {
  passed: boolean;
  issues: StrategistAtomizationGuardCheckIssue[];
  metrics: {
...
export interface StrategistAtomizationAdversarialGuardScenario {
  id: string;
  description: string;
  build: (record: StrategistAtomizationRunRecord) => StrategistAtomizationRunRecord;
  expectRejected: true;
}

export const FORGE_STRATEGIST_ATOMIZATION_GUARD_CONTROLS_V1: ForgeStrategistAtomizationGuardControls =
  {
    atom: "P03-B03-A09",
    adversarial: {
      rejectTamperedRecords: true,
...
export function getForgeStrategistAtomizationGuardControls(): ForgeStrategistAtomizationGuardControls {
  return FORGE_STRATEGIST_ATOMIZATION_GUARD_CONTROLS_V1;
}

function parseStrategistAtomizationIsoDurationMs(startedAt: string, completedAt: string): number {
...
export function summarizeStrategistAtomizationTelemetry(
  telemetry: StrategistAtomizationProbeTelemetry[],
): {
  suiteDurationMs: number;
  maxProbeDurationMs: number;
...
export function detectStrategistAtomizationEvidenceSummaryMismatch(
  record: StrategistAtomizationRunRecord,
): string | null {
  let alignedCount = 0;
  for (const item of record.evidence) {
...
export function detectStrategistAtomizationFalseAlignment(
  record: StrategistAtomizationRunRecord,
): string[] {
...
export function validateStrategistAtomizationSafety(
  record: StrategistAtomizationRunRecord,
  controls: ForgeStrategistAtomizationGuardControls = getForgeStrategistAtomizationGuardControls(),
): StrategistAtomizationGuardCheckIssue[] {
  const issues: StrategistAtomizationGuardCheckIssue[] = [];
  for (const item of record.evidence) {
...
export function validateStrategistAtomizationPerformance(
  record: StrategistAtomizationRunRecord,
  controls: ForgeStrategistAtomizationGuardControls = getForgeStrategistAtomizationGuardControls(),
): StrategistAtomizationGuardCheckIssue[] {
  const issues: StrategistAtomizationGuardCheckIssue[] = [];
  const { suiteDurationMs, maxProbeDurationMs } = summarizeStrategistAtomizationTelemetry(
    record.telemetry,
  );
  const wallClockMs = parseStrategistAtomizationIsoDurationMs(
...
export function validateStrategistAtomizationCost(
  totalCostUsd: number,
  llmCalls: number,
  controls: ForgeStrategistAtomizationGuardControls = getForgeStrategistAtomizationGuardControls(),
): StrategistAtomizationGuardCheckIssue[] {
...
export function buildStrategistAtomizationAdversarialGuardScenarios(): StrategistAtomizationAdversarialGuardScenario[] {
  return [
    {
...
export function runStrategistAtomizationAdversarialGuardChecks(
  fixtureRecord: StrategistAtomizationRunRecord,
  contract: StrategistAtomizationContract = getActiveStrategistAtomizationContract(),
  ...
  for (const scenario of scenarios) {
    ...
    const validation = validateStrategistAtomizationRunRecord(tampered, contract);
    const falseAlignment = detectStrategistAtomizationFalseAlignment(tampered);
    const summaryMismatch = detectStrategistAtomizationEvidenceSummaryMismatch(tampered);
...
export function validateForgeStrategistAtomizationGuard(
  record: StrategistAtomizationRunRecord,
  options: {
    totalCostUsd?: number;
    llmCalls?: number;
    contract?: StrategistAtomizationContract;
    controls?: ForgeStrategistAtomizationGuardControls;
  } = {},
): StrategistAtomizationGuardCheckResult {
  const controls = options.controls ?? getForgeStrategistAtomizationGuardControls();
  const contract = options.contract ?? getActiveStrategistAtomizationContract();
  const totalCostUsd = options.totalCostUsd ?? 0;
  const llmCalls = options.llmCalls ?? 0;
  const issues: StrategistAtomizationGuardCheckIssue[] = [];

  issues.push(...validateStrategistAtomizationPerformance(record, controls));
  issues.push(...validateStrategistAtomizationCost(totalCostUsd, llmCalls, controls));
  issues.push(...validateStrategistAtomizationSafety(record, controls));

  const falseAlignment = detectStrategistAtomizationFalseAlignment(record);
  ...
  const summaryMismatch = detectStrategistAtomizationEvidenceSummaryMismatch(record);
  if (summaryMismatch) {
    issues.push({
      domain: "adversarial",
  ...
  const telemetrySummary = summarizeStrategistAtomizationTelemetry(record.telemetry);
  const wallClockMs = parseStrategistAtomizationIsoDurationMs(
    record.provenance.startedAt,
    record.provenance.completedAt,
  );
