...
// ─── Probe regression detection (P02-B08-A08) ────────────────────────────────

export interface VisionerScoringProbeRegressionReport {
...
/**
 * Compare visioner scoring run records and detect probe alignment regressions.
 * A regression = probe aligned in prior run but misaligned in current run.
 */
export function detectVisionerScoringProbeRegression(
  prior: VisionerScoringRunRecord,
  current: VisionerScoringRunRecord,
): VisionerScoringProbeRegressionReport {
...
// ─── Guard controls (P02-B08-A09 foundation, used by A08 regression gate) ───

export interface ForgeVisionerScoringGuardControls {
  atom: string;
  adversarial: {
...
export interface VisionerScoringGuardCheckIssue {
  domain: "adversarial" | "performance" | "cost" | "safety";
  code: string;
  detail: string;
}

export interface VisionerScoringGuardCheckResult {
  passed: boolean;
  issues: VisionerScoringGuardCheckIssue[];
  metrics: {
...
export interface VisionerScoringAdversarialGuardScenario {
  id: string;
  description: string;
  build: (record: VisionerScoringRunRecord) => VisionerScoringRunRecord;
  expectRejected: true;
}

export const FORGE_VISIONER_SCORING_GUARD_CONTROLS_V1: ForgeVisionerScoringGuardControls = {
  atom: "P02-B08-A09",
  adversarial: {
    rejectTamperedRecords: true,
    rejectFalseAlignment: true,
...
export function getForgeVisionerScoringGuardControls(): ForgeVisionerScoringGuardControls {
  return FORGE_VISIONER_SCORING_GUARD_CONTROLS_V1;
}

function parseVisionerScoringIsoDurationMs(startedAt: string, completedAt: string): number {
...
export function summarizeVisionerScoringTelemetry(telemetry: VisionerScoringProbeTelemetry[]): {
  suiteDurationMs: number;
  maxProbeDurationMs: number;
} {
...
export function detectVisionerScoringEvidenceSummaryMismatch(
  record: VisionerScoringRunRecord,
): string | null {
  let alignedCount = 0;
  for (const item of record.evidence) {
...
export function detectVisionerScoringFalseAlignment(record: VisionerScoringRunRecord): string[] {
...
export function validateVisionerScoringSafety(
  record: VisionerScoringRunRecord,
  controls: ForgeVisionerScoringGuardControls = getForgeVisionerScoringGuardControls(),
): VisionerScoringGuardCheckIssue[] {
  const issues: VisionerScoringGuardCheckIssue[] = [];
  for (const item of record.evidence) {
...
export function validateVisionerScoringPerformance(
  record: VisionerScoringRunRecord,
  controls: ForgeVisionerScoringGuardControls = getForgeVisionerScoringGuardControls(),
): VisionerScoringGuardCheckIssue[] {
  const issues: VisionerScoringGuardCheckIssue[] = [];
  const { suiteDurationMs, maxProbeDurationMs } = summarizeVisionerScoringTelemetry(record.telemetry);
  const wallClockMs = parseVisionerScoringIsoDurationMs(
...
export function validateVisionerScoringCost(
  totalCostUsd: number,
  llmCalls: number,
  controls: ForgeVisionerScoringGuardControls = getForgeVisionerScoringGuardControls(),
): VisionerScoringGuardCheckIssue[] {
...
export function buildVisionerScoringAdversarialGuardScenarios(): VisionerScoringAdversarialGuardScenario[] {
  return [
    {
...
export function runVisionerScoringAdversarialGuardChecks(
  fixtureRecord: VisionerScoringRunRecord,
  contract: VisionerScoringContract = getActiveVisionerScoringContract(),
  ...
  for (const scenario of scenarios) {
    ...
    const validate = resolveVisionerScoringRunRecordValidator(tampered);
    const validation = validate(tampered, contract);
    const falseAlignment = detectVisionerScoringFalseAlignment(tampered);
    const summaryMismatch = detectVisionerScoringEvidenceSummaryMismatch(tampered);
...
export function validateForgeVisionerScoringGuard(
  record: VisionerScoringRunRecord,
  options: {
    totalCostUsd?: number;
    llmCalls?: number;
    contract?: VisionerScoringContract;
    controls?: ForgeVisionerScoringGuardControls;
  } = {},
): VisionerScoringGuardCheckResult {
  const controls = options.controls ?? getForgeVisionerScoringGuardControls();
  const contract = options.contract ?? getActiveVisionerScoringContract();
  const totalCostUsd = options.totalCostUsd ?? 0;
  const llmCalls = options.llmCalls ?? 0;
  const issues: VisionerScoringGuardCheckIssue[] = [];

  issues.push(...validateVisionerScoringPerformance(record, controls));
  issues.push(...validateVisionerScoringCost(totalCostUsd, llmCalls, controls));
  issues.push(...validateVisionerScoringSafety(record, controls));

  const falseAlignment = detectVisionerScoringFalseAlignment(record);
  ...
  const summaryMismatch = detectVisionerScoringEvidenceSummaryMismatch(record);
  if (summaryMismatch) {
    issues.push({
      domain: "adversarial",
  ...
  const telemetrySummary = summarizeVisionerScoringTelemetry(record.telemetry);
  const wallClockMs = parseVisionerScoringIsoDurationMs(
    record.provenance.startedAt,
    record.provenance.completedAt,
  );
...
export interface VisionerScoringBlockGateEvidence {
  blockId: string;
  atom: string;
  sealedAt: string;
...
export interface VisionerScoringBlockHandoffContract {
  version: string;
  atom: string;
  sourceBlock: {
    blockId: string;
  ...
  sealedArtifacts: {
    ...
    harnessVersion: string;
    probeCount: number;
    visionerScoringCategories: readonly VisionerScoringCategory[];
    sourceVisionerAlternativeBlockGateAtom: string;
  };
  prerequisites: readonly string[];
  entryCriteria: {
    description: string;
    requiresBlockGatePass: true;
    visionerScoringRecordRequired: true;
  };
}

export const FORGE_P02_B08_BLOCK_GATE_V1: ForgeBlockGateDefinition = {
  version: "1.0.0",
  atom: "P02-B08-A10",
  blockId: "P02-B08",
  ...
  checks: [
    { id: "fixture_contract_alignment", atomId: "P02-B08-A01", description: "Visioner scoring baseline aligns with typed contract and P02-B07 block gate handoff" },
    { id: "typed_contract_coverage", atomId: "P02-B08-A02", description: "Contract declares measurable probes for all visioner scoring categories" },
    { id: "probe_matrix_aligned", atomId: "P02-B08-A03", description: "Visioner scoring probe matrix executes with zero unexpected mismatches" },
    { id: "boundary_disposition_coverage", atomId: "P02-B08-A04", description: "Contract covers observed, gap, failure, recovery and NO-GO dispositions" },
    { id: "failure_recovery_nogo", atomId: "P02-B08-A05", description: "Failure, recovery and NO-GO probes are declared and exercised" },
    { id: "evidence_telemetry_provenance", atomId: "P02-B08-A06", description: "Run record carries evidence, telemetry and provenance" },
    { id: "property_and_fuzz", atomId: "P02-B08-A07", description: "Structural property and fuzz validation reject tampered inputs" },
    { id: "regression_gate", atomId: "P02-B08-A08", description: "Regression gate passes on canonical visioner scoring matrix" },
    { id: "guard_controls", atomId: "P02-B08-A09", description: "Adversarial, performance, cost and safety guard controls pass" },
    { id: "block_gate_sealed", atomId: "P02-B08-A10", description: "Block gate evidence sealed with valid B09 handoff contract" },
  ] satisfies readonly ForgeBlockGateCheck[],
};

export const FORGE_P02_B08_TO_B09_HANDOFF_V1: VisionerScoringBlockHandoffContract = {
  version: "1.0.0",
  atom: "P02-B08-A10",
  sourceBlock: {
    blockId: "P02-B08",
    title: "Vizyon scoring ve trade-off",
    completedAtoms: FORGE_P02_B08_BLOCK_GATE_V1.requiredAtomIds,
  },
  targetBlock: {
    blockId: "P02-B09",
    title: "Kullanıcı approval ve steering",
    entryAtom: "P02-B09-A01",
  },
  sealedArtifacts: {
    fixtureVersion: "1.0.0",
    contractVersion: FORGE_VISIONER_SCORING_CONTRACT_V1.version,
    harnessVersion: FORGE_VISIONER_SCORING_VERSION,
    probeCount: summarizeVisionerScoringContractCoverage(FORGE_VISIONER_SCORING_CONTRACT_V1).totalProbes,
    visionerScoringCategories: VISIONER_SCORING_CATEGORIES,
    sourceVisionerAlternativeBlockGateAtom: "P02-B07-A10",
  },
  prerequisites: [
    "Visioner scoring contract v1 with measurable trade-off, recovery and guard probes",
    "Versioned visioner scoring baseline aligned to contract probe matrix and sealed P02-B07 block gate",
    "Evidence, telemetry and provenance run records",
    "Regression and guard gates integrated with orchestrator verification",
    "Sealed P02-B07 visioner alternative block gate referenced by sourceVisionerAlternativeBlockGateAtom",
  ],
  entryCriteria: {
    description:
      "P02-B09-A01 formalizes user approval and steering using sealed visioner scoring artifacts",
    requiresBlockGatePass: true,
    visionerScoringRecordRequired: true,
  },
};

export function getForgeP02B08BlockGate(): ForgeBlockGateDefinition {
  return FORGE_P02_B08_BLOCK_GATE_V1;
}

export function getForgeP02B08ToB09Handoff(): VisionerScoringBlockHandoffContract {
  return FORGE_P02_B08_TO_B09_HANDOFF_V1;
}

export function validateVisionerScoringBlockHandoffContract(
  handoff: VisionerScoringBlockHandoffContract,
  evidence: Pick<VisionerScoringBlockGateEvidence, "probeCount" | "regressionPassed" | "guardPassed">,
  contract: VisionerScoringContract = getActiveVisionerScoringContract(),
  ...
  if (handoff.sealedArtifacts.visionerScoringCategories.length !== VISIONER_SCORING_CATEGORIES.length) {
    issues.push("handoff visionerScoringCategories incomplete");
  }
  if (handoff.targetBlock.entryAtom !== "P02-B09-A01") {
...
export function buildVisionerScoringBlockGateEvidence(
  atomSeals: ForgeBlockAtomSeal[],
  regressionPassed: boolean,
  guardPassed: boolean,
  probeCount: number,
  gitCommit?: string,
  blockId = FORGE_P02_B08_BLOCK_GATE_V1.blockId,
): VisionerScoringBlockGateEvidence {
  const handoff = getForgeP02B08ToB09Handoff();
  const handoffValid = validateVisionerScoringBlockHandoffContract(handoff, {
    probeCount,
    regressionPassed,
    guardPassed,
  }).valid;
