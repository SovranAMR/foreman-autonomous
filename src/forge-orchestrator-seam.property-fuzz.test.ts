...
import { loadOrchestratorSeamBaseline, runOrchestratorSeamFailureRecoverySliceWithRecord } from "./forge-orchestrator-seam.probe.js";
import {
  FORGE_ORCHESTRATOR_SEAM_CONTRACT_V1,
...
describe("Forge Orchestrator Seam — P01-B09-A07 property checks", () => {
  it("passes all structural properties on canonical contract", () => {
    const result = runOrchestratorSeamPropertyChecks(FORGE_ORCHESTRATOR_SEAM_CONTRACT_V1);
    assert.equal(result.allPassed, true, result.failed.map(f => `${f.propertyId}: ${f.detail}`).join("\n"));
    assert.equal(result.passed, result.total);
    assert.equal(result.total, 8);
  });

  it("createOrchestratorSeamFuzzRng is deterministic for reproducible fuzz seeds", () => {
    const rngA = createOrchestratorSeamFuzzRng(1337);
    const rngB = createOrchestratorSeamFuzzRng(1337);
    const seqA = Array.from({ length: 5 }, () => rngA());
    const seqB = Array.from({ length: 5 }, () => rngB());
    assert.deepEqual(seqA, seqB);
    assert.notDeepEqual(seqA, Array.from({ length: 5 }, () => createOrchestratorSeamFuzzRng(1338)()));
  });
});

describe("Forge Orchestrator Seam — P01-B09-A07 fixture fuzz validation", () => {
  it("rejects all deterministic fixture mutations", () => {
    ...
    for (const seed of [42, 99, 20260718]) {
      const fuzz = runOrchestratorSeamFuzzValidation(fixture, contract, seed, 24);
...
describe("Forge Orchestrator Seam — P01-B09-A07 run record fuzz validation", () => {
  it("accepts valid failure/recovery record and rejects corrupted mutations", () => {
    ...
    assert.equal(
      validateOrchestratorSeamFailureRecoveryRunRecord(record, contract).valid,
      true,
      validateOrchestratorSeamFailureRecoveryRunRecord(record, contract).issues.map(i => i.detail).join("\n"),
    );

    const fuzz = runOrchestratorSeamRunRecordFuzzValidation(record, contract);
  ...
  it("validates full contract run record and rejects tampered evidence/telemetry/provenance", () => {
    ...
    const telemetry = probeIds.map((id, index) => {
      const probe = contract.probes.find(p => p.id === id)!;
      return buildOrchestratorSeamProbeTelemetry(id, probe.category, index, index * 0.05);
    });
    ...
    assert.equal(validateOrchestratorSeamRunRecord(record, contract).valid, true);
