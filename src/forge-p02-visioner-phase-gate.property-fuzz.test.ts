...
import {
  FORGE_VISIONER_PHASE_GATE_CONTRACT_V1,
  ...
  createVisionerPhaseGateFuzzRng,
...
describe("Forge Visioner Phase Gate — P02-B10-A07 property checks", () => {
  it("passes all structural properties on canonical contract", () => {
    const result = runVisionerPhaseGatePropertyChecks(FORGE_VISIONER_PHASE_GATE_CONTRACT_V1);
    assert.equal(result.allPassed, true, result.failed.map(f => `${f.propertyId}: ${f.detail}`).join("\n"));
    assert.equal(result.passed, result.total);
    assert.equal(result.total, 8);
  });

  it("createVisionerPhaseGateFuzzRng is deterministic for reproducible fuzz seeds", () => {
    const rngA = createVisionerPhaseGateFuzzRng(1337);
    const rngB = createVisionerPhaseGateFuzzRng(1337);
    const seqA = Array.from({ length: 5 }, () => rngA());
    const seqB = Array.from({ length: 5 }, () => rngB());
    assert.deepEqual(seqA, seqB);
    assert.notDeepEqual(seqA, Array.from({ length: 5 }, () => createVisionerPhaseGateFuzzRng(1338)()));
  });
});

describe("Forge Visioner Phase Gate — P02-B10-A07 fixture fuzz validation", () => {
  it("rejects all deterministic fixture mutations", () => {
    ...
    for (const seed of [42, 99, 20260719]) {
      const fuzz = runVisionerPhaseGateFuzzValidation(fixture, contract, seed, 24);
...
describe("Forge Visioner Phase Gate — P02-B10-A07 run record fuzz validation", () => {
  it("accepts valid failure/recovery record and rejects corrupted mutations", () => {
    const contract = getActiveVisionerPhaseGateContract();
    const record = runVisionerPhaseGateFailureRecoverySliceWithRecord();

    assert.equal(
      validateVisionerPhaseGateFailureRecoveryRunRecord(record, contract).valid,
      true,
      validateVisionerPhaseGateFailureRecoveryRunRecord(record, contract).issues.map(i => i.detail).join("\n"),
    );

    const fuzz = runVisionerPhaseGateRunRecordFuzzValidation(record, contract);
  ...
  it("validates full contract run record and rejects tampered evidence/telemetry/provenance", () => {
    ...
    const telemetry = probeIds.map((id, index) => {
      const probe = contract.probes.find(p => p.id === id)!;
      return buildVisionerPhaseGateProbeTelemetry(id, probe.category, index, index * 0.05);
    });
    ...
    const record = buildVisionerPhaseGateRunRecord(provenance, evidence, telemetry);

    assert.equal(validateVisionerPhaseGateRunRecord(record, contract).valid, true);
