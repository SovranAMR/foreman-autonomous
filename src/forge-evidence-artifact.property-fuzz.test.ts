...
import { loadEvidenceArtifactBaseline, runEvidenceArtifactFailureRecoverySliceWithRecord } from "./forge-evidence-artifact.probe.js";
import {
...
describe("Forge Evidence Artifact — P01-B08-A07 property checks", () => {
  it("passes all structural properties on canonical contract", () => {
    const result = runEvidenceArtifactPropertyChecks(FORGE_EVIDENCE_ARTIFACT_CONTRACT_V1);
    assert.equal(result.allPassed, true, result.failed.map(f => `${f.propertyId}: ${f.detail}`).join("\n"));
    assert.equal(result.passed, result.total);
    assert.equal(result.total, 8);
  });

  it("createEvidenceArtifactFuzzRng is deterministic for reproducible fuzz seeds", () => {
    const rngA = createEvidenceArtifactFuzzRng(1337);
    const rngB = createEvidenceArtifactFuzzRng(1337);
    const seqA = Array.from({ length: 5 }, () => rngA());
    const seqB = Array.from({ length: 5 }, () => rngB());
    assert.deepEqual(seqA, seqB);
    assert.notDeepEqual(seqA, Array.from({ length: 5 }, () => createEvidenceArtifactFuzzRng(1338)()));
  });
});

describe("Forge Evidence Artifact — P01-B08-A07 fixture fuzz validation", () => {
  it("rejects all deterministic fixture mutations", () => {
    ...
    for (const seed of [42, 99, 20260718]) {
      const fuzz = runEvidenceArtifactFuzzValidation(fixture, contract, seed, 24);
...
describe("Forge Evidence Artifact — P01-B08-A07 run record fuzz validation", () => {
  it("accepts valid failure/recovery record and rejects corrupted mutations", () => {
    ...
    assert.equal(
      validateEvidenceArtifactFailureRecoveryRunRecord(record, contract).valid,
      true,
      validateEvidenceArtifactFailureRecoveryRunRecord(record, contract).issues.map(i => i.detail).join("\n"),
    );

    const fuzz = runEvidenceArtifactRunRecordFuzzValidation(record, contract);
  ...
  it("validates full contract run record and rejects tampered evidence/telemetry/provenance", () => {
    ...
    const telemetry = probeIds.map((id, index) => {
      const probe = contract.probes.find(p => p.id === id)!;
      return buildEvidenceArtifactProbeTelemetry(id, probe.category, index, index * 0.05);
    });
    ...
    assert.equal(validateEvidenceArtifactRunRecord(record, contract).valid, true);
