...
// ─── Block gate and handoff (P04-B09-A10) ─────────────────────────────────────

export interface ResearcherResearchToWorkerHandoffBlockGateEvidence {
  blockId: string;
  atom: string;
  sealedAt: string;
...
export interface ResearcherResearchToWorkerHandoffBlockHandoffContract {
  version: string;
  atom: string;
  sourceBlock: {
    blockId: string;
  ...
  sealedArtifacts: {
    ...
    harnessVersion: string;
    probeCount: number;
    researchToWorkerHandoffCategories: readonly ResearcherResearchToWorkerHandoffCategory[];
    sourceBlockGateAtom: string;
  };
  ...
  entryCriteria: {
    ...
    researchToWorkerHandoffRecordRequired: true;
  };
}

export const FORGE_P04_B09_BLOCK_GATE_V1: ForgeBlockGateDefinition = {
  version: "1.0.0",
  atom: "P04-B09-A10",
  blockId: "P04-B09",
  ...
  checks: [
    {
      ...
      atomId: "P04-B09-A01",
      description:
        "Research-to-worker handoff baseline aligns with typed contract and P04-B08 block gate handoff",
    },
    {
      id: "typed_contract_coverage",
      atomId: "P04-B09-A02",
      description: "Contract declares measurable probes for all research-to-worker handoff categories",
    },
    {
      id: "probe_matrix_aligned",
      atomId: "P04-B09-A03",
      description: "Research-to-worker handoff probe matrix executes with zero unexpected mismatches",
    },
    {
    ...
    },
    {
      id: "regression_gate",
      atomId: "P04-B09-A08",
      description: "Regression gate passes on canonical research-to-worker handoff matrix",
    },
    {
      id: "guard_controls",
      atomId: "P04-B09-A09",
...
export const FORGE_P04_B09_TO_B10_HANDOFF_V1: ResearcherResearchToWorkerHandoffBlockHandoffContract = {
  version: "1.0.0",
  atom: "P04-B09-A10",
  sourceBlock: {
    blockId: "P04-B09",
    title: "Research-to-worker handoff",
    completedAtoms: FORGE_P04_B09_BLOCK_GATE_V1.requiredAtomIds,
  },
  targetBlock: {
    blockId: "P04-B10",
    title: "Araştırmacı phase gate",
    entryAtom: "P04-B10-A01",
  },
  sealedArtifacts: {
    fixtureVersion: "1.0.0",
    contractVersion: FORGE_RESEARCHER_RESEARCH_TO_WORKER_HANDOFF_CONTRACT_V1.version,
    harnessVersion: FORGE_RESEARCHER_RESEARCH_TO_WORKER_HANDOFF_VERSION,
    probeCount: summarizeResearcherResearchToWorkerHandoffContractCoverage(
      FORGE_RESEARCHER_RESEARCH_TO_WORKER_HANDOFF_CONTRACT_V1,
    ).totalProbes,
    researchToWorkerHandoffCategories: RESEARCHER_RESEARCH_TO_WORKER_HANDOFF_CATEGORIES,
    sourceBlockGateAtom: "P04-B08-A10",
  },
  prerequisites: [
    "Research-to-worker handoff contract v1 with measurable handoff signal, worker context and guard probes",
    "Versioned research-to-worker handoff baseline aligned to contract probe matrix and sealed P04-B08 block gate",
    "Evidence, telemetry and provenance run records",
    "Regression and guard gates integrated with orchestrator verification",
    "Sealed P04-B08 spike falsification block gate referenced by sourceBlockGateAtom",
  ],
  entryCriteria: {
    description:
      "P04-B10-A01 formalizes researcher phase gate using sealed research-to-worker handoff artifacts",
    requiresBlockGatePass: true,
    researchToWorkerHandoffRecordRequired: true,
  },
};

export function getForgeP04B09BlockGate(): ForgeBlockGateDefinition {
  return FORGE_P04_B09_BLOCK_GATE_V1;
}

export function getForgeP04B09ToB10Handoff(): ResearcherResearchToWorkerHandoffBlockHandoffContract {
  return FORGE_P04_B09_TO_B10_HANDOFF_V1;
}

export function validateResearcherResearchToWorkerHandoffBlockHandoffContract(
  handoff: ResearcherResearchToWorkerHandoffBlockHandoffContract,
  evidence: Pick<
    ResearcherResearchToWorkerHandoffBlockGateEvidence,
  ...
  if (handoff.sealedArtifacts.harnessVersion !== FORGE_RESEARCHER_RESEARCH_TO_WORKER_HANDOFF_VERSION) {
    issues.push(
      `handoff harnessVersion=${handoff.sealedArtifacts.harnessVersion} active=${FORGE_RESEARCHER_RESEARCH_TO_WORKER_HANDOFF_VERSION}`,
    );
  }
  if (
    ...
    issues.push("handoff researchToWorkerHandoffCategories incomplete");
  }
  if (handoff.sealedArtifacts.sourceBlockGateAtom !== "P04-B08-A10") {
    issues.push(`unexpected source block gate atom: ${handoff.sealedArtifacts.sourceBlockGateAtom}`);
  }
  if (handoff.targetBlock.entryAtom !== "P04-B10-A01") {
...
export function buildResearcherResearchToWorkerHandoffBlockGateEvidence(
  atomSeals: ForgeBlockAtomSeal[],
  regressionPassed: boolean,
  guardPassed: boolean,
  probeCount: number,
  gitCommit?: string,
  blockId = FORGE_P04_B09_BLOCK_GATE_V1.blockId,
): ResearcherResearchToWorkerHandoffBlockGateEvidence {
  const handoffValid = validateResearcherResearchToWorkerHandoffBlockHandoffContract(
