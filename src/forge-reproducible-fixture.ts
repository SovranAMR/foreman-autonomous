...
// ─── Probe regression detection (P01-B07-A08) ────────────────────────────────

export interface ReproducibleFixtureProbeRegressionReport {
...
/**
 * Compare reproducible fixture run records and detect probe alignment regressions.
 * A regression = probe aligned in prior run but misaligned in current run.
 */
export function detectReproducibleFixtureProbeRegression(
  prior: ReproducibleFixtureRunRecord,
  current: ReproducibleFixtureRunRecord,
): ReproducibleFixtureProbeRegressionReport {
...
// ─── Guard controls (P01-B07-A09 foundation, used by A08 regression gate) ────

export interface ForgeReproducibleFixtureGuardControls {
  atom: string;
  adversarial: {
...
export interface ReproducibleFixtureGuardCheckIssue {
  domain: "adversarial" | "performance" | "cost" | "safety";
  code: string;
  detail: string;
}

export interface ReproducibleFixtureGuardCheckResult {
  passed: boolean;
  issues: ReproducibleFixtureGuardCheckIssue[];
  metrics: {
...
export interface ReproducibleFixtureAdversarialGuardScenario {
  id: string;
  description: string;
  build: (record: ReproducibleFixtureRunRecord) => ReproducibleFixtureRunRecord;
  expectRejected: true;
}

export const FORGE_REPRODUCIBLE_FIXTURE_GUARD_CONTROLS_V1: ForgeReproducibleFixtureGuardControls = {
  atom: "P01-B07-A09",
  adversarial: {
    rejectTamperedRecords: true,
    rejectFalseAlignment: true,
...
export function getForgeReproducibleFixtureGuardControls(): ForgeReproducibleFixtureGuardControls {
  return FORGE_REPRODUCIBLE_FIXTURE_GUARD_CONTROLS_V1;
}

function parseReproducibleFixtureIsoDurationMs(startedAt: string, completedAt: string): number {
...
export function summarizeReproducibleFixtureTelemetry(telemetry: ReproducibleFixtureProbeTelemetry[]): {
  suiteDurationMs: number;
  maxProbeDurationMs: number;
} {
...
export function detectReproducibleFixtureEvidenceSummaryMismatch(
  record: ReproducibleFixtureRunRecord,
): string | null {
  let alignedCount = 0;
  for (const item of record.evidence) {
...
export function detectReproducibleFixtureFalseAlignment(record: ReproducibleFixtureRunRecord): string[] {
...
export function validateReproducibleFixtureSafety(
  record: ReproducibleFixtureRunRecord,
  controls: ForgeReproducibleFixtureGuardControls = getForgeReproducibleFixtureGuardControls(),
): ReproducibleFixtureGuardCheckIssue[] {
  const issues: ReproducibleFixtureGuardCheckIssue[] = [];
  for (const item of record.evidence) {
...
export function validateReproducibleFixturePerformance(
  record: ReproducibleFixtureRunRecord,
  controls: ForgeReproducibleFixtureGuardControls = getForgeReproducibleFixtureGuardControls(),
): ReproducibleFixtureGuardCheckIssue[] {
  const issues: ReproducibleFixtureGuardCheckIssue[] = [];
  const { suiteDurationMs, maxProbeDurationMs } = summarizeReproducibleFixtureTelemetry(record.telemetry);
  const wallClockMs = parseReproducibleFixtureIsoDurationMs(
...
export function validateReproducibleFixtureCost(
  totalCostUsd: number,
  llmCalls: number,
  controls: ForgeReproducibleFixtureGuardControls = getForgeReproducibleFixtureGuardControls(),
): ReproducibleFixtureGuardCheckIssue[] {
...
export function buildReproducibleFixtureAdversarialGuardScenarios(): ReproducibleFixtureAdversarialGuardScenario[] {
  return [
    {
...
export function runReproducibleFixtureAdversarialGuardChecks(
  fixtureRecord: ReproducibleFixtureRunRecord,
  contract: ReproducibleFixtureContract = getActiveReproducibleFixtureContract(),
  ...
  for (const scenario of scenarios) {
    ...
    const validation = validateReproducibleFixtureRunRecord(tampered, contract);
    const falseAlignment = detectReproducibleFixtureFalseAlignment(tampered);
    const summaryMismatch = detectReproducibleFixtureEvidenceSummaryMismatch(tampered);
...
export function validateForgeReproducibleFixtureGuard(
  record: ReproducibleFixtureRunRecord,
  options: {
    totalCostUsd?: number;
    llmCalls?: number;
    contract?: ReproducibleFixtureContract;
    controls?: ForgeReproducibleFixtureGuardControls;
  } = {},
): ReproducibleFixtureGuardCheckResult {
  const controls = options.controls ?? getForgeReproducibleFixtureGuardControls();
  const contract = options.contract ?? getActiveReproducibleFixtureContract();
  const totalCostUsd = options.totalCostUsd ?? 0;
  const llmCalls = options.llmCalls ?? 0;
  const issues: ReproducibleFixtureGuardCheckIssue[] = [];

  issues.push(...validateReproducibleFixturePerformance(record, controls));
  issues.push(...validateReproducibleFixtureCost(totalCostUsd, llmCalls, controls));
  issues.push(...validateReproducibleFixtureSafety(record, controls));

  const falseAlignment = detectReproducibleFixtureFalseAlignment(record);
  ...
  const summaryMismatch = detectReproducibleFixtureEvidenceSummaryMismatch(record);
  if (summaryMismatch) {
    issues.push({
      domain: "adversarial",
  ...
  const adversarial = runReproducibleFixtureAdversarialGuardChecks(record, contract);
  ...
  const telemetrySummary = summarizeReproducibleFixtureTelemetry(record.telemetry);
  const wallClockMs = parseReproducibleFixtureIsoDurationMs(
    record.provenance.startedAt,
    record.provenance.completedAt,
  );
...
// ─── Block gate and handoff (P01-B07-A10) ───────────────────────────────────

export interface ReproducibleFixtureBlockGateEvidence {
  blockId: string;
  atom: string;
  sealedAt: string;
...
export interface ReproducibleFixtureBlockHandoffContract {
  version: string;
  atom: string;
  sourceBlock: {
    blockId: string;
  ...
  sealedArtifacts: {
    ...
    harnessVersion: string;
    probeCount: number;
    reproducibleFixtureCategories: readonly ReproducibleFixtureCategory[];
    sourceBenchmarkEvalAtom: string;
  };
  ...
  entryCriteria: {
    ...
    reproducibleFixtureRecordRequired: true;
  };
}

export const FORGE_P01_B07_BLOCK_GATE_V1: ForgeBlockGateDefinition = {
  version: "1.0.0",
  atom: "P01-B07-A10",
  blockId: "P01-B07",
  ...
  checks: [
    { id: "fixture_contract_alignment", atomId: "P01-B07-A01", description: "Reproducible fixture baseline aligns with typed contract" },
    { id: "typed_contract_coverage", atomId: "P01-B07-A02", description: "Contract declares measurable probes for all reproducible fixture categories" },
    { id: "probe_matrix_aligned", atomId: "P01-B07-A03", description: "Reproducible fixture probe matrix executes with zero unexpected mismatches" },
    { id: "boundary_disposition_coverage", atomId: "P01-B07-A04", description: "Contract covers observed, gap, failure, recovery and NO-GO dispositions" },
    { id: "failure_recovery_nogo", atomId: "P01-B07-A05", description: "Failure, recovery and NO-GO probes are declared and exercised" },
    { id: "evidence_telemetry_provenance", atomId: "P01-B07-A06", description: "Run record carries evidence, telemetry and provenance" },
    { id: "property_and_fuzz", atomId: "P01-B07-A07", description: "Structural property and fuzz validation reject tampered inputs" },
    { id: "regression_gate", atomId: "P01-B07-A08", description: "Regression gate passes on canonical reproducible fixture matrix" },
    { id: "guard_controls", atomId: "P01-B07-A09", description: "Adversarial, performance, cost and safety guard controls pass" },
    { id: "block_gate_sealed", atomId: "P01-B07-A10", description: "Block gate evidence sealed with valid B08 handoff contract" },
  ] satisfies readonly ForgeBlockGateCheck[],
};

export const FORGE_P01_B07_TO_B08_HANDOFF_V1: ReproducibleFixtureBlockHandoffContract = {
  version: "1.0.0",
  atom: "P01-B07-A10",
  sourceBlock: {
    blockId: "P01-B07",
    title: "Reproducible fixture sistemi",
    completedAtoms: FORGE_P01_B07_BLOCK_GATE_V1.requiredAtomIds,
  },
  targetBlock: {
    blockId: "P01-B08",
    title: "Evidence ve artifact şeması",
    entryAtom: "P01-B08-A01",
  },
  sealedArtifacts: {
    fixtureVersion: "1.0.0",
    contractVersion: FORGE_REPRODUCIBLE_FIXTURE_CONTRACT_V1.version,
    harnessVersion: FORGE_REPRODUCIBLE_FIXTURE_VERSION,
    probeCount: summarizeReproducibleFixtureContractCoverage(FORGE_REPRODUCIBLE_FIXTURE_CONTRACT_V1).totalProbes,
    reproducibleFixtureCategories: REPRODUCIBLE_FIXTURE_CATEGORIES,
    sourceBenchmarkEvalAtom: "P01-B06-A10",
  },
  prerequisites: [
    "Reproducible fixture contract v1 with measurable versioning, integrity, deterministic load and baseline link probes",
    "Versioned reproducible fixture baseline aligned to contract probe matrix",
    "Evidence, telemetry and provenance run records",
    "Regression and guard gates integrated with orchestrator verification",
    "Sealed P01-B06 benchmark eval harness artifacts referenced by sourceBenchmarkEval",
  ],
  entryCriteria: {
    description:
      "B08-A01 formalizes evidence and artifact schema using sealed reproducible fixture system artifacts",
    requiresBlockGatePass: true,
    reproducibleFixtureRecordRequired: true,
  },
};

export function getForgeP01B07BlockGate(): ForgeBlockGateDefinition {
  return FORGE_P01_B07_BLOCK_GATE_V1;
}

export function getForgeP01B07ToB08Handoff(): ReproducibleFixtureBlockHandoffContract {
  return FORGE_P01_B07_TO_B08_HANDOFF_V1;
}

export function validateReproducibleFixtureBlockHandoffContract(
  handoff: ReproducibleFixtureBlockHandoffContract,
  evidence: Pick<ReproducibleFixtureBlockGateEvidence, "probeCount" | "regressionPassed" | "guardPassed">,
  contract: ReproducibleFixtureContract = getActiveReproducibleFixtureContract(),
  ...
  if (handoff.sealedArtifacts.reproducibleFixtureCategories.length !== REPRODUCIBLE_FIXTURE_CATEGORIES.length) {
    issues.push("handoff reproducibleFixtureCategories incomplete");
  }
  if (handoff.targetBlock.entryAtom !== "P01-B08-A01") {
...
export function buildReproducibleFixtureBlockGateEvidence(
  atomSeals: ForgeBlockAtomSeal[],
  regressionPassed: boolean,
  guardPassed: boolean,
  probeCount: number,
  gitCommit?: string,
  blockId = FORGE_P01_B07_BLOCK_GATE_V1.blockId,
): ReproducibleFixtureBlockGateEvidence {
  const handoff = getForgeP01B07ToB08Handoff();
  const handoffValid = validateReproducibleFixtureBlockHandoffContract(handoff, {
    probeCount,
    regressionPassed,
    guardPassed,
  }).valid;
